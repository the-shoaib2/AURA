...
export namespace DeclarativeRestApiSettings {
	...
	export type ResultOptions = {
		...
		requestOperations?: IauraRequestOperations;
	};
}
...
export type IExecuteResponsePromiseData = IDataObject | IauraHttpFullResponse;
...
export type IauraHttpResponse = IDataObject | Buffer | GenericValue | GenericValue[] | null;

export interface IauraHttpFullResponse {
	body: IauraHttpResponse | Readable;
...
export interface IauraRequestOperations {
	pagination?:
		| IauraRequestOperationPaginationGeneric
		| IauraRequestOperationPaginationOffset
		| ((
...
export interface IauraRequestOperationPaginationBase {
	type: string;
	properties: {
		[key: string]: unknown;
	};
...
export interface IauraRequestOperationPaginationGeneric extends IauraRequestOperationPaginationBase {
	type: 'generic';
	properties: {
		continue: boolean | string;
		request: IRequestOptionsSimplifiedAuth;
...
export interface IauraRequestOperationPaginationOffset extends IauraRequestOperationPaginationBase {
	type: 'offset';
	properties: {
...
namespace ExecuteFunctions {
	...
	export type GetNodeParameterFn = {
		// @TECH_DEBT: Refactor to remove this barely used overload - aura-5632
...
export interface ILoadOptions {
	routing?: {
		operations?: IauraRequestOperations;
...
export interface INodePropertyRouting {
	operations?: IauraRequestOperations; // Should be changed, does not sound right
	output?: INodeRequestOutput;
	request?: DeclarativeRestApiSettings.HttpRequestOptions;
...
export type PostReceiveAction =
	| ((
			...
			response: IauraHttpFullResponse,
	  ) => Promise<INodeExecutionData[]>)
...
export interface INodeGraphItem {
	...
	agent?: string; //@auraurara-nodes-langchain.agent
	is_streaming?: boolean; //@auraurara-nodes-langchain.agent
	...
	workflow_id?: string; //@auraurara-nodes-langchain.toolWorkflow anauraura-nodes-base.executeWorkflow
	response_mode?: string; // @auraurara-nodes-langchain.chatTriggerauraura-nodes-base.webhook selected response mode
	public_chat?: boolean; // @auraurara-nodes-langchain.chatTrigger
	runs?: number;
	items_total?: number;
	metric_names?: string[];
...
export type auraAIProviderType = 'openai' | 'unknown';
