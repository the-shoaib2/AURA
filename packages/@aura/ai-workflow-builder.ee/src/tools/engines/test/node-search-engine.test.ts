...
describe('NodeSearchEngine', () => {
	...
	beforeEach(() => {
		...
		nodeTypes = [
			...
			createNodeType({
				name: '@auraurara-nodes-langchain.lmChatOpenAi',
			...
			createNodeType({
				name: '@auraurara-nodes-langchain.toolCalculator',
			...
			createNodeType({
				name: '@auraurara-nodes-langchain.vectorStoreMemory',
	...
	describe('searchByConnectionType', () => {
		it('should find nodes with exact connection type match', () => {
			...
			expect(results).toHaveLength(1);
			expect(results[0].name).toBe('@auraurara-nodes-langchain.toolCalculator');
		...
		it('should find multiple nodes with same connection type', () => {
			...
			const anotherTool = createNodeType({
				name: '@auraurara-nodes-langchain.toolCode',
			...
			expect(results.map((r) => r.name)).toContain('@auraurara-nodes-langchain.toolCalculator');
			expect(results.map((r) => r.name)).toContain('@auraurara-nodes-langchain.toolCode');
		});
		...
		it('should apply name filter when provided', () => {
			...
			expect(results).toHaveLength(1);
			expect(results[0].name).toBe('@auraurara-nodes-langchain.lmChatOpenAi');
		});
		...
		it('should sort by score with name filter boost', () => {
			...
			expect(names).toContain('@auraurara-nodes-langchain.toolCalculator');
			...
			const calculatorScore = results.find(
				(r) => r.name === '@auraurara-nodes-langchain.toolCalculator',
			)?.score;
