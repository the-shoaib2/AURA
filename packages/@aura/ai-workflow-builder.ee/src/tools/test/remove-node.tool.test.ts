	...
	describe('invoke', () => {
		...
		it('should handle removing node with AI connections', async () => {
			const existingWorkflow = createWorkflow([
				...
				createNode({ id: 'node2', name: 'AI Agent', type: '@auraurara-nodes-langchain.agent' }),
				createNode({
					id: 'node3',
		...
		it('should handle different connection types', async () => {
			const existingWorkflow = createWorkflow([
				createNode({
					id: 'node1',
					name: 'Vector Store',
					type: '@auraurara-nodes-langchain.vectorStore',
				}),
				createNode({
					id: 'node2',
					name: 'Embeddings',
				...
				createNode({
					...
					type: '@auraurara-nodes-langchain.documentLoader',
				}),
			]);
