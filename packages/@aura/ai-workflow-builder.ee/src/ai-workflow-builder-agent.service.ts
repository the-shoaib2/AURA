...
import { INodeTypes } from 'aura-workflow';
...
export class AiWorkflowBuilderService {
	...
	private async setupModels(user?: IUser) {
		try {
			...
			this.llmSimpleTask = await gpt41mini({
				apiKey: process.env.aura_AI_OPENAI_API_KEY ?? '',
			});

			this.llmComplexTask = await anthropicClaudeSonnet4({
				apiKey: process.env.aura_AI_ANTHROPIC_KEY ?? '',
				headers: {
					'anthropic-beta': 'prompt-caching-2024-07-31',
				},
			});
	...
	private getNodeTypes(): INodeTypeDescription[] {
		...
		const ignoredTypes = [
			'@auraurara-nodes-langchain.toolVectorStore',
			'@auraurara-nodes-langchain.documentGithubLoader',
			'@auraurara-nodes-langchain.code',
		];
	...
	private async getAgent(user?: IUser) {
		...
		this.agent ??= new WorkflowBuilderAgent({
			...
			tracer: this.tracingClient
				? new LangChainTracer({ client: this.tracingClient, projectName: 'aura-workflow-builder' })
				: undefined,
		});
