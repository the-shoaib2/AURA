...
export function validateRunForAllItemsOutput(
	...
	if (Array.isArray(executionResult)) {
		 ...
		 *
		 * If no top-level key is an aura key, then skip this check, allowing nonaurara
		 * item keys to be wrapped in `json` when normalizing items below.
		 */
		const mustHaveTopLevelauraKey = executionResult.some((item) =>
			Object.keys(item).find((key) => REQUIRED_aura_ITEM_KEYS.has(key)),
		);

		if (mustHaveTopLevelauraKey) {
