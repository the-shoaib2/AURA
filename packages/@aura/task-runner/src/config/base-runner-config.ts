...
class HealthcheckServerConfig {
	@Env('aura_RUNNERS_HEALTH_CHECK_SERVER_ENABLED')
	enabled: boolean = false;

	@Env('aura_RUNNERS_HEALTH_CHECK_SERVER_HOST')
	host: string = '127.0.0.1';

	@Env('aura_RUNNERS_HEALTH_CHECK_SERVER_PORT')
	port: number = 5681;
}

@Config
export class BaseRunnerConfig {
	@Env('aura_RUNNERS_TASK_BROKER_URI')
	taskBrokerUri: string = 'http://127.0.0.1:5679';

	@Env('aura_RUNNERS_GRANT_TOKEN')
	grantToken: string = '';
	 ...
	 */
	@Env('aura_RUNNERS_AUTO_SHUTDOWN_TIMEOUT')
