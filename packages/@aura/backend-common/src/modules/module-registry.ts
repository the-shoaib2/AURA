...
import type { EntityClass, ModuleSettings } from '@aura/decorators';
...
export class ModuleRegistry {
	...
	async loadModules(modules?: ModuleName[]) {
		...
		try {
			// docker + tests
			const auraPackagePath = require.resolve(aurara/package.json');
			const auraRoot = path.dirnameauraraPackagePath);
			const srcDirExists = existsSync(path.join(auraRoot, 'src'));
			const dir = process.env.NODE_ENV === 'test' && srcDirExists ? 'src' : 'dist';
			modulesDir = path.join(auraRoot, dir, 'modules');
		} catch {
			// local dev
			// aura binary is inside the bin folder, so we need to go up two levels
