...
describe('getConnectedTools', () => {
	...
	beforeEach(() => {
		...
		mockauraTool = new auraTool(mockExecuteFunctions as unknown as ISupplyDataFunctions, {
			name: 'Dummy Tool',
	...
	it('should convert auraTool to dynamic tool when convertStructuredTool is true', async () => {
		...
		mockauraTool.asDynamicTool = asDynamicToolSpy;

		mockExecuteFunctions.getInputConnectionData = jest.fn().mockResolvedValue([mockauraTool]);
	...
	it('should not convert auraTool when convertStructuredTool is false', async () => {
		mockExecuteFunctions.getInputConnectionData = jest.fn().mockResolvedValue([mockauraTool]);

		const tools = await getConnectedTools(mockExecuteFunctions, true, false);
		expect(tools[0]).toBe(mockauraTool);
	});
