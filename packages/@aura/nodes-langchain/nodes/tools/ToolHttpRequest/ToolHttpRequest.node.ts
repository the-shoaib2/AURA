...
import { auraTool } from '@utils/auraTool';
...
export class ToolHttpRequest implements INodeType {
		...
		codex: {
			...
			resources: {
				primaryDocumentation: [
					{
						url: 'https://docs.aura.io/integrations/builtin/cluster-nodes/sub-nodesaurara-nodes-langchain.toolhttprequest/',
					},
				],
			},
		},
	...
	async supplyData(this: ISupplyDataFunctions, itemIndex: number): Promise<SupplyData> {
		...
		let tool: DynamicTool | auraTool;

		// If the node version is 1.1 or higher, we use the auraTool wrapper:
