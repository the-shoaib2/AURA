...
import {
	...
	type auraOutputFixingParser,
} from '@utils/output_parsers/auraOutputParser';
		...
		describe('Version 1.1 and below', () => {
          ...
          "required": ["user", "timestamp"]
				...
				const { response } = (await outputParser.supplyData.call(thisArg, 0)) as {
					response: auraStructuredOutputParser;
				};
				const outputObject = {
					output: {
						user: {
	...
	describe('Auto-Fix', () => {
		...
		describe('Configuration', () => {
			it('should use default prompt when none specified', async () => {
				...
				const { response } = (await outputParser.supplyData.call(thisArg, 0)) as {
					response: auraOutputFixingParser;
				};
		...
		describe('Parsing', () => {
			let mockStructuredOutputParser: MockProxy<auraStructuredOutputParser>;

			beforeEach(() => {
				mockStructuredOutputParser = mock<auraStructuredOutputParser>();

				jest
					.spyOn(auraStructuredOutputParser, 'fromZodJsonSchema')
