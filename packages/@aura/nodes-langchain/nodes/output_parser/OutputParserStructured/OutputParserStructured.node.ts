...
export class OutputParserStructured implements INodeType {
		...
		codex: {
			...
			resources: {
				primaryDocumentation: [
					{
						url: 'https://docs.aura.io/integrations/builtin/cluster-nodes/sub-nodesaurara-nodes-langchain.outputparserstructured/',
					},
				],
			},
		},
  ...
  "properties": {
	...
	async supplyData(this: ISupplyDataFunctions, itemIndex: number): Promise<SupplyData> {
		...
		let outputParser;
		try {
			outputParser = await auraStructuredOutputParser.fromZodJsonSchema(
				zodSchema,
				nodeVersion,
				this,
			);
		...
		const parser = new auraOutputFixingParser(
			this,
			model,
			outputParser,
