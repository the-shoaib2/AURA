...
import { auraLlmTracing } from '../llms/auraLlmTracing';
import { auraNonEstimatingTracing } from '../llms/auraNonEstimatingTracing';
		...
		codex: {
			...
			resources: {
				primaryDocumentation: [
					{
						url: 'https://docs.aura.io/integrations/builtin/cluster-nodes/sub-nodesaurara-nodes-langchain.modelselector/',
					},
				],
			},
		},
		...
		for (let i = 0; i < rules.length; i++) {
			...
			if (conditionsMet) {
				...
				for (const currentCallback of originalCallbacks) {
					if (currentCallback instanceof auraLlmTracing) {
						currentCallback.setParentRunIndex(this.getNextRunIndex());
					}
				}
				const modelSelectorTracing = new auraNonEstimatingTracing(this);
