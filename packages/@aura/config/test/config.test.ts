...
describe('GlobalConfig', () => {
	...
	const defaultConfig: GlobalConfig = {
		...
		security: {
			...
			blockFileAccessToauraFiles: true,
	...
	describe('string unions', () => {
		it('on invalid value, should warn and fall back to default value', () => {
			process.env = {
				aura_RUNNERS_MODE: 'non-existing-mode',
			...
			expect(consoleWarnMock).toHaveBeenCalledWith(
				expect.stringContaining(
					"Invalid value for aura_RUNNERS_MODE - Invalid enum value. Expected 'internal' | 'external', received 'non-existing-mode'. Falling back to default value.",
				),
			);
