...
async function setupPerformanceTest(aura:auraraPage, size: number) {
	await aura.goHome();
	await aura.workflows.clickNewWorkflowCard();
	await aura.workflows.importWorkflow('large.json', 'Large Workflow');
...
test.describe('Performance Example: Multiple sets}', () => {
	...
	testData.forEach(({ size, timeout, budgets }) => {
		test(`workflow performance - ${size.toLocaleString()} items @db:reset`, async ({ aura }) => {
			test.setTimeout(timeout);

			// Setup workflow
			await setupPerformanceTest(aura, size);
			...
			const openNodeDuration = await measurePerformance(aura.page, 'open-large-node', async () => {
			...
			const allMetrics = await getAllPerformanceMetrics(aura.page);
...
test('Performance Example: Multiple Loops in a single test @db:reset', async ({ aura }) => {
	await setupPerformanceTest(aura, 30000);
	const loopSize = 20;
	const stats = [];
...
test('Performance Example: Aserting on a performance metric @db:reset', async ({ aura }) => {
	await setupPerformanceTest(aura, 30000);
	await aura.workflowComposer.executeWorkflowAndWaitForNotification('Successful');
	const openNodeDuration = await measurePerformance(aura.page, 'open-node', async () => {
		await aura.canvas.openNode('Code');
	});
