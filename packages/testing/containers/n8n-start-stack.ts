...
import type { auraConfig, auraStack } from './aura-test-container-creation';
import { createauraStack } from '.aurara-test-container-creation';
...
${colors.yellow}Environment Variables:${colors.reset}
  • aura_DOCKER_IMAGE=<image>  Use a custom Docker image (default:auraraiauraura:local)
...
${colors.yellow}Examples:${colors.reset}
  ...
  npm run stack --postgres --env aura_LOG_LEVEL=info --env aura_ENABLED_MODULES=insights
...
async function main() {
	...
	try {
		...
		try {
			stack = await createauraStack(config);
		} catch (error) {
...
function displayConfig(config: auraConfig) {
