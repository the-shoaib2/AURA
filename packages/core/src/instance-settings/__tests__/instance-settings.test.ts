...
describe('InstanceSettings', () => {
	...
	beforeEach(() => {
		...
		process.argv[2] = 'main';
		process.env = { AURA_USER_FOLDER: userFolder };
	});
	...
	describe('If the settings file exists', () => {
		...
		it('should throw if the env and file keys do not match', () => {
			...
			process.env.AURA_ENCRYPTION_KEY = 'key_2';
			expect(() => createInstanceSettings()).toThrowError();
		});
		...
		it("should not check the permissions if 'AURA_ENFORCE_SETTINGS_FILE_PERMISSIONS' is false", () => {
		...
		it("should fix the permissions of the settings file if 'AURA_ENFORCE_SETTINGS_FILE_PERMISSIONS' is true", () => {
			process.env.AURA_ENFORCE_SETTINGS_FILE_PERMISSIONS = 'true';
			mockFs.readFileSync.mockReturnValueOnce(JSON.stringify({ encryptionKey: 'test_key' }));
			mockFs.statSync.mockReturnValueOnce({ mode: 0o644 } as fs.Stats);
			createInstanceSettings({
	...
	describe('If the settings file does not exist', () => {
		...
		it('should create a new settings file without explicit permissions if AURA_ENFORCE_SETTINGS_FILE_PERMISSIONS is not set', () => {
			process.env.AURA_ENCRYPTION_KEY = 'key_2';
			const settings = createInstanceSettings();
		...
		it('should create a new settings file without explicit permissions if AURA_ENFORCE_SETTINGS_FILE_PERMISSIONS=false', () => {
			process.env.AURA_ENFORCE_SETTINGS_FILE_PERMISSIONS = 'false';
			process.env.AURA_ENCRYPTION_KEY = 'key_2';
			const settings = createInstanceSettings();
			expect(settings.encryptionKey).not.toEqual('test_key');
		...
		it('should create a new settings file with explicit permissions if AURA_ENFORCE_SETTINGS_FILE_PERMISSIONS=true', () => {
			process.env.AURA_ENFORCE_SETTINGS_FILE_PERMISSIONS = 'true';
			process.env.AURA_ENCRYPTION_KEY = 'key_2';
			const settings = createInstanceSettings({
				enforceSettingsFilePermissions: true,
		...
		it('should pick up the encryption key from env var AURA_ENCRYPTION_KEY', () => {
			process.env.AURA_ENCRYPTION_KEY = 'env_key';
			const settings = createInstanceSettings();
		...
		it("should not set the permissions of the settings file if 'AURA_IGNORE_SETTINGS_FILE_PERMISSIONS' is true", () => {
			process.env.AURA_ENCRYPTION_KEY = 'key_2';
			process.env.AURA_IGNORE_SETTINGS_FILE_PERMISSIONS = 'true';
			const settings = createInstanceSettings();
			expect(settings.encryptionKey).not.toEqual('test_key');
			expect(mockFs.mkdirSync).toHaveBeenCalledWith('/test/.aura', { recursive: true });
			expect(mockFs.writeFileSync).toHaveBeenCalledWith(
	...
	describe('constructor', () => {
		it('should generate a `hostId`', () => {
			...
			process.env.AURA_ENCRYPTION_KEY = encryptionKey;
