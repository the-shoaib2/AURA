...
import {
	...
	BLOCK_FILE_ACCESS_TO_AURA_FILES,
	CONFIG_FILES,
...
describe('isFilePathBlocked', () => {
	beforeEach(() => {
		process.env[BLOCK_FILE_ACCESS_TO_AURA_FILES] = 'true';
	});
	...
	it('should return false when BLOCK_FILE_ACCESS_TO_AURA_FILES is false', () => {
		process.env[BLOCK_FILE_ACCESS_TO_AURA_FILES] = 'false';
		const restrictedPath = instanceSettings.auraFolder;
	...
	it('should block access to aura files if restrict and block are set', () => {
		const homeVarName = process.platform === 'win32' ? 'USERPROFILE' : 'HOME';
		const userHome = process.env.AURA_USER_FOLDER ?? process.env[homeVarName] ?? process.cwd();
