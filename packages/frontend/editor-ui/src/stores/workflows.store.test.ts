...
import type { PushPayload, FrontendSettings } from '@aura/api-types';
	...
	describe('updateNodeExecutionData', () => {
		...
		it('should add node error event and track errored executions', async () => {
			...
			expect(track).toHaveBeenCalledWith('Manual exec errored', {
				...
				node_graph_string:
					'{"node_types":["aura-nodes-base.set"],"node_connections":[],"nodes":{"0":{"id":"554c7ff4-7ee2-407c-8931-e34234c5056a","type":aurara-nodes-base.set","version":3.4,"position":[680,180]}},"notes":{},"is_pinned":false}',
			});
		});
