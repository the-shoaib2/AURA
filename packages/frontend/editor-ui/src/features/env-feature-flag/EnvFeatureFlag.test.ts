...
import type { FrontendSettings, auraEnvFeatFlags, auraEnvFeatFlagValue } from '@aura/api-types';
...
describe('EnvFeatureFlag', () => {
	...
	beforeEach(() => {
		Object.keys(import.meta.env).forEach((key) => {
			if (key.startsWith('aura_ENV_FEAT_')) {
				delete (import.meta.env as auraEnvFeatFlags)[key as keyof auraEnvFeatFlags];
			}
		});
		createTestingPinia();
	...
	test.each<[auraEnvFeatFlagValue, Uppercase<string>, boolean]>([
		...
		(value, flagName, shouldRender) => {
			const envKey: keyof auraEnvFeatFlags = AURAra_ENV_FEAT_${flagName}`;
	...
	it('should work with different flag names', () => {
		settingsStore.settings.envFeatureFlags = {
			AURAa_ENV_FEAT_WORKFLOW_DIFFS: 'true',
			AURAa_ENV_FEAT_ANOTHER_FEATURE: 'false',
		};
	...
	describe('runtime vs build-time priority', () => {
		it('should prioritize runtime settings over build-time env vars', () => {
			...
			(import.meta.env as auraEnvFeatFlags)AURAra_ENV_FEAT_TEST_FLAG = 'true';

			// Set runtime setting to override
			settingsStore.settings.envFeatureFlags = {
				AURAa_ENV_FEAT_TEST_FLAG: 'false',
			};
