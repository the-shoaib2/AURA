...
import { auraButton, auraHeading, auraIconButton, auraRadioButtons, auraText } from '@aura/design-system';
			...
			<div :class="$style.header">
				<div :class="$style.headerLeft">
					<auraIconButton
						...
						@click="closeDialog"
					></auraIconButton>
					<auraHeading tag="h1" size="xlarge">
						{{
									...
									<div>
										...
										<ul v-if="activeTab === 'settings'">
											<li v-for="setting in settingsDiff" :key="setting.name">
												<auraText color="text-dark" size="medium" tag="div" bold>{{
				...
				<div :class="$style.workflowDiff">
					<div :class="$style.workflowDiffPanel">
						<template v-if="sourceWorkFlow.state.value">
							<auraText color="text-dark" size="small" :class="$style.sourceBadge">
								<auraIcon v-if="sourceWorkFlow.state.value.remote" icon="git-branch" />
								{{
							...
							<template v-else>
								<div :class="$style.emptyWorkflow">
									<template v-if="targetWorkFlow.state.value?.remote">
										<auraText color="text-dark" size="large"> Deleted workflow </auraText>
										<auraText color="text-base"> The workflow was deleted on the database </auraText>
									</template>
									<template v-else>
										<auraText color="text-dark" size="large"> Deleted workflow </auraText>
										<auraText color="text-base"> The workflow was deleted on remote </auraText>
									</template>
								</div>
							</template>
						</template>
					...
					<div :class="$style.workflowDiffPanel">
						<template v-if="targetWorkFlow.state.value">
							<auraText color="text-dark" size="small" :class="$style.sourceBadge">
								<auraIcon v-if="targetWorkFlow.state.value.remote" icon="git-branch" />
								{{
							...
							<template v-else>
								<div :class="$style.emptyWorkflow">
									...
									<template v-else>
										<auraText color="text-dark" size="large"> Deleted workflow </auraText>
										<auraText color="text-base"> The workflow was deleted on the data base </auraText>
									</template>
								</div>
							</template>
						</template>
...
.tabs {
	display: flex;
	flex-direction: row;
	:global(.aura-radio-button) {
		flex: 1;
	}
	:global(.aura-radio-button > div) {
...
.modified {
	...
	&[data-node-type='aura-nodes-base.stickyNote'],
	&[data-node-type='aura-nodes-base.manualTrigger'] {
		&::before {
			left: auto;
			right: 0px;
