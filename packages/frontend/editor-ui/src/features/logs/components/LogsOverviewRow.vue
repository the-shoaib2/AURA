...
import { auraButton, auraIcon, auraIconButton, auraText } from '@aura/design-system';
...
<template>
	...
	>
		...
		/>
		<auraText v-if="!isCompact" tag="div" color="text-light" size="small" :class="$style.timeTook">
			<I18nT v-if="timeText !== undefined" :keypath="statusTextKeyPath" scope="global">
				<template #status>
					<auraText :color="isError ? 'danger' : undefined" :bold="isError" size="small">
						<AnimatedSpinner v-if="isRunning" :class="$style.statusTextIcon" />
						<auraIcon v-else-if="isWaiting" icon="status-waiting" :class="$style.statusTextIcon" />
						<auraIcon v-else-if="isError" icon="triangle-alert" :class="$style.statusTextIcon" />
						{{ statusText }}
					</auraText>
				</template>
		...
		<template v-if="isCompact && !hasChildren">
			...
			<auraIcon v-else-if="isWaiting" icon="status-waiting" :class="$style.statusIcon" />
		</template>
		<auraButton
