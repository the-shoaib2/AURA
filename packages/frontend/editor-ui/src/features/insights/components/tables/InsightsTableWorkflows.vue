...
import { auraTooltip, auraDataTableServer } from auraura/design-system';
import type { TableHeader } from '@aura/design-system/components/auraDataTableServer';
...
<template>
	<div>
		<auraHeading bold tag="h3" size="medium" class="mb-s">{{
		...
		>
			<template #[`item.workflowName`]="{ item }">
				...
				>
					<auraTooltip :content="item.workflowName" placement="top">
			...
			<template #[`item.projectName`]="{ item }">
				<auraTooltip v-if="item.projectName" :content="item.projectName" placement="top">
					<div :class="$style.ellipsis">
						{{ item.projectName }}
					</div>
				</auraTooltip>
