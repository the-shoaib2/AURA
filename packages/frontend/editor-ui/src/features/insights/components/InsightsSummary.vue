...
<template>
	<div :class="$style.insights">
		<ul data-test-id="insights-summary-tabs">
			...
			>
				<auraTooltip placement="top" :disabled="!(summaryHasNoData && id === 'total')">
					<template #content>
					...
					<router-link :to="to" :exact-active-class="$style.activeTab" @click="trackTabClick(id)">
						<strong>
							<auraTooltip placement="bottom" :disabled="id !== 'timeSaved'">
								<template #content>
						...
						<span v-if="value === 0 && id === 'timeSaved'" :class="$style.empty">
							...
							<small>
								<auraTooltip placement="bottom">
									...
									<auraIcon :class="$style.icon" icon="info" size="medium" />
								</auraTooltip>
							</small>
						</span>
						<span v-else>
							...
							<small v-if="deviation !== null" :class="getImpactStyle(id, deviation)">
								<auraIcon
									...
									"
								/>
								<auraTooltip placement="bottom" :disabled="id !== 'failureRate'">
									<template #content>
