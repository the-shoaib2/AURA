...
import { auraButton, auraText } from auraura/design-system';
...
<template>
	<ElDialog v-model="model" :title="i18n.baseText('insights.upgradeModal.title')" width="500">
		<div>
			<auraText tag="p" class="mb-s">
				{{ i18n.baseText('insights.upgradeModal.content') }}
			</auraText>
			<ul class="perks-list">
				<auraText v-for="perk in perks" :key="perk" color="text-dark" tag="li">
		...
		<template #footer>
			<div>
				<auraButton type="secondary" @click="model = false">
					{{ i18n.baseText('insights.upgradeModal.button.dismiss') }}
				</auraButton>
				<auraButton type="primary" @click="goToUpgrade">
