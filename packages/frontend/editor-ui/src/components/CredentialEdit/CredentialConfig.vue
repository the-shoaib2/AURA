...
import InlineAskAssistantButton from '@aura/design-system/components/InlineAskAssistantButton/InlineAskAssistantButton.vue';
...
onBeforeMount(async () => {
	...
	const key = `aura-nodes-base.credentials.${props.credentialType.name}`;
...
const documentationUrl = computed(() => {
	...
	if (url.hostname === DOCS_DOMAIN) {
		url.searchParams.set('utm_source', 'aura_app');
...
<template>
	<aura-callout v-if="isManaged" theme="warning" icon="triangle-alert">
	...
	<div v-else>
		<div :class="$style.config" data-test-id="node-credentials-config-container">
			...
			<template v-if="credentialPermissions.update">
				<aura-notice v-if="documentationUrl && credentialProperties.length" theme="warning">
					{{ i18n.baseText('credentialEdit.credentialConfig.needHelpFillingOutTheseFields') }}
					<span class="ml-4xs">
						<aura-link :to="documentationUrl" size="small" bold @click="onDocumentationUrlClick">
			...
			<aura-text v-if="isMissingCredentials" color="text-base" size="medium">
			...
			<EnterpriseEdition :features="[EnterpriseEditionFeature.ExternalSecrets]">
				<template #fallback>
					<aura-info-tip class="mt-s">
						{{ i18n.baseText('credentialEdit.credentialConfig.externalSecrets') }}
						<aura-link bold :to="i18n.baseText('settings.externalSecrets.docs')" size="small">
