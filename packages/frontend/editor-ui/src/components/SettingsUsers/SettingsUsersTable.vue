...
import type { TableHeader, TableOptions } from '@aura/design-system/components/auraDataTableServer';
...
import type { UsersInfoProps } from '@aura/design-system/components/auraUserInfo/UserInfo.vue';
...
const headers = ref<Array<TableHeader<Item>>>([
	...
	},
	{
		...
		value(row) {
			return {
				...row,
				// TODO: Fix auraDataTableServer so it doesn't break sorting when `value` is of mixed type (for example, string or null)
		...
		>
			<template #[`item.name`]="{ value }">
				<div class="pt-xs pb-xs">
					<auraUserInfo v-bind="value as UsersInfoProps" />
				</div>
			</template>
			<template #[`item.role`]="{ item }">
				...
				/>
				<auraText v-else color="text-dark">{{ roles[item.role ?? ROLE.Default].label }}</auraText>
			</template>
