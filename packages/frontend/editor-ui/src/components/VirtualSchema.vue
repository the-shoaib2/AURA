...
<template>
	<div :class="['run-data-schema', 'full-height', props.compact ? 'compact' : '']">
		<div v-if="noSearchResults" class="no-results">
			<auraText tag="h3" size="large">{{ i18n.baseText('ndv.search.noNodeMatch.title') }}</auraText>
			<auraText>
			...
			>
				<template #default="{ item, index, active }">
					...
					>
						...
						<auraTooltip v-else-if="item.type === 'icon'" :content="item.tooltip" placement="top">
							<auraIcon size="small" :icon="item.icon" class="icon" />
						</auraTooltip>

						<div
							v-else-if="item.type === 'notice'"
							v-aura-html="item.message"
							class="notice"
