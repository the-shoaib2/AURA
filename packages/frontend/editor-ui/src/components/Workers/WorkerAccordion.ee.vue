...
<template>
	<div :class="['accordion', $style.container]">
		<div :class="{ [$style.header]: true, [$style.expanded]: expanded }" @click="toggle">
			<aura-icon :icon="icon" :color="iconColor" size="small" class="mr-2xs" />
			<aura-text :class="$style.headerText" color="text-base" size="small" align="left" bold>
				<slot name="title"></slot>
			</aura-text>
			<aura-icon :icon="expanded ? 'chevron-up' : 'chevron-down'" bold />
		</div>
