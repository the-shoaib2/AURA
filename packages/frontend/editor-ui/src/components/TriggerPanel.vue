		...
		<transition name="fade" mode="out-in">
			...
			<div v-else-if="isListeningForEvents" key="listening">
				...
				<div v-if="isWebhookNode">
					<aura-text tag="div" size="large" color="text-dark" class="mb-2xs" bold>{{
						i18n.baseText('ndv.trigger.webhookNode.listening')
					}}</aura-text>
				...
				<div v-else>
					...
					<div v-if="displayChatButton">
						<aura-button class="mb-xl" @click="openWebhookUrl()">
			...
			<div v-else key="default">
				<div v-if="isActivelyPolling" class="mb-xl">
					<aura-spinner type="ring" />
				</div>

				<div :class="$style.action">
					<div :class="$style.header">
						<aura-heading v-if="header" tag="h1" bold>
							{{ header }}
						</aura-heading>
						<aura-text v-if="subheader">
				...
				<aura-text v-if="activationHint" size="small" @click="onLinkClick">
					<span v-aura-html="activationHint"></span>&nbsp;
				</aura-text>
				<aura-link
					v-if="activationHint && executionsHelp"
					size="small"
					@click="expandExecutionHelp"
					>{{ i18n.baseText('ndv.trigger.moreInfo') }}</aura-link
				>
				<aura-info-accordion
					v-if="executionsHelp"
					ref="help"
