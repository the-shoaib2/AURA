...
import { auraLoading } from 'aurara/design-system';
import type { EventBus } from '@aura/utils/event-bus';
import { createEventBus } from '@aura/utils/event-bus';
import type { INodeParameterResourceLocator, NodeParameterValue } from 'aura-workflow';
...
<template>
	...
	>
		...
		<div v-if="filterable && !errorView" :class="$style.searchInput" @keydown="onKeyDown">
			...
			>
				<template #prefix>
					<aura-icon :class="$style.searchIcon" icon="search" />
				</template>
			</auraInput>
		</div>
		...
		>
			...
			>
				<div :class="$style.resourceNameContainer">
					...
					<aura-icon :class="$style.addResourceIcon" icon="plus" />
				</div>
			</div>
			<div
			...
			>
				<div :class="$style.resourceNameContainer">
					...
					<span v-if="result.isArchived">
						<auraBadge class="ml-3xs" theme="tertiary" bold data-test-id="workflow-archived-tag">
			...
			<div v-if="loading && !errorView">
				<div v-for="i in 3" :key="i" :class="$style.loadingItem">
					<auraLoading :class="$style.loader" variant="p" :rows="1" />
				</div>
			</div>
		</div>
		<template #reference>
