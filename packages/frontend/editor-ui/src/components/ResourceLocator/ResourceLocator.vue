...
import { auraNotice } from 'aurara/design-system';
			...
			>
				...
				<div :class="$style.inputContainer" data-test-id="rlc-input-container">
					...
					<div v-else-if="urlValue" :class="$style.openResourceLink">
						<aura-link theme="text" @click.stop="openResource(urlValue)">
							<aura-icon icon="external-link" :title="getLinkAlt(valueToDisplay)" />
						</aura-link>
					</div>
				</div>
			</div>
