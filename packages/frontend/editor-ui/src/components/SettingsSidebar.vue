...
const sidebarMenuItems = computed<IMenuItem[]>(() => {
	const menuItems: IMenuItem[] = [
		...
		},
		{
			id: 'settings-api',
			icon: 'plug',
			label: i18n.baseText('settings.aurapi'),
			position: 'top',
...
<template>
	<div :class="$style.container">
		<aura-menu :items="sidebarMenuItems">
			<template #header>
				<div :class="$style.returnButton" data-test-id="settings-back" @click="emit('return')">
					<i class="mr-xs">
						<aura-icon icon="arrow-left" />
					</i>
					<aura-heading size="large" :bold="true">{{ i18n.baseText('settings') }}<aurara-heading>
				</div>
			</template>
			<template #menuSuffix>
				<div :class="$style.versionContainer">
					<aura-link size="small" @click="uiStore.openModal(ABOUT_MODAL_KEY)">
