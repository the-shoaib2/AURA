...
<template>
	...
	>
		...
		<div v-show="!binaryDataDisplayVisible">
			...
			>
				<div :class="$style.runSelectorInner">
					...
					<auraTooltip v-if="canLinkRuns" placement="right">
						<template #content>
			...
			<div v-if="props.calloutMessage || $slots['callout-message']" :class="$style.hintCallout">
				<auraCallout theme="info" data-test-id="run-data-callout">
					<slot name="callout-message">
						<auraText vaurara-html="props.calloutMessage" size="small"></auraText>
					</slot>
				</auraCallout>
			</div>
			<NodeSettingsHint
			...
			>
				<auraText vaurara-html="hint.message" size="small"></auraText>
			</auraCallout>
		...
		<div ref="dataContainerRef" :class="$style.dataContainer" data-test-id="ndv-data-container">
			...
			>
				<div v-if="!props.compact" :class="$style.spinner">
					<auraSpinner type="ring" />
				</div>
				<auraText>{{ executingMessage }}</auraText>
			</div>
			...
			<div v-else-if="editMode.enabled" :class="$style.editMode">
				...
				<div :class="$style.editModeFooter">
					<auraInfoTip :bold="false" :class="$style.editModeFooterInfotip">
						{{ i18n.baseText('runData.editor.copyDataInfo') }}
						<auraLink :to="DATA_EDITING_DOCS_URL" size="small">
			...
			<div
				...
				:class="$style.center"
			>
				<div :class="$style.spinner"><auraSpinner type="ring" /></div>
				<auraText color="text-dark" size="large">
					{{ i18n.baseText('runData.trimmedData.loading') }}
				</auraText>
			</div>

			<div v-else-if="isTrimmedManualExecutionDataItem" :class="$style.center">
				<auraText bold color="text-dark" size="large">
			...
			<div v-else-if="hasNodeRun && hasRunError" :class="$style.stretchVertically">
				<auraText v-if="isPaneTypeInput" :class="$style.center" size="large" tag="p" bold>
					{{
			...
			>
				<div v-if="search">
					<auraText tag="h3" size="large">{{ i18n.baseText('ndv.search.noMatch.title') }}</auraText>
					<auraText>
			...
			<div
				...
				:class="$style.center"
			>
				<auraText :bold="true" color="text-dark" size="large">{{ tooMuchDataTitle }}</auraText>
				<auraText align="center" tag="div"
					><span
						v-aura-html="
		...
		/>
		<auraBlockUi :show="blockUI" :class="$style.uiBlocker" />
	</div>
</template>
