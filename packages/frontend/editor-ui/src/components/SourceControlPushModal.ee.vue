...
<template>
	...
	>
		<template #header>
			...
			>
				<div :class="[$style.filters]">
					...
					<auraPopover trigger="click" width="304" style="align-self: normal">
						<template #reference>
							<auraButton
								icon="funnel"
								type="tertiary"
							...
							>
								<auraBadge v-if="filterCount" theme="primary" class="mr-4xs">
									{{ filterCount }}
								</auraBadge>
							</auraButton>
						</template>
			...
			<template v-if="usersStore.currentUser && usersStore.currentUser.role">
				<template
					...
					"
				>
					<auraCallout v-if="!projectAdminCalloutDismissed" theme="secondary" class="mt-s">
			...
			<div style="display: flex; height: 100%">
				<div :class="$style.tabs">
					<template v-for="tab in tabs" :key="tab.value">
						...
						>
							<div>{{ tab.label }}</div>
							<auraText tag="div" color="text-light">
				...
				<div style="flex: 1">
					<div :class="[$style.table]">
						...
						<div style="flex: 1; overflow: hidden">
							<auraInfoTip v-if="!activeDataSourceFiltered.length" class="p-xs" :bold="false">
								{{ filtersNoResultText }}
							</auraInfoTip>
							<DynamicScroller
									...
									>
										...
										>
											...
											<span :class="[$style.badges]">
												...
												<auraBadge :theme="getStatusTheme(file.status)">
		...
		<template #footer>
			<auraNotice v-if="userNotices.length" :compact="false" class="mt-0">
				<auraText bold size="medium">Changes to variables, tags and folders </auraText>
				<br />
				<template v-for="{ title, content } in userNotices" :key="title">
					<auraText bold size="small"> {{ title }}</auraText>
					<auraText size="small"> : {{ content }}. </auraText>
				</template>
			</auraNotice>
