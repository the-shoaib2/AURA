...
<template>
	...
	>
			...
			<div v-else :class="$style.container">
				...
				<enterprise-edition :features="[EnterpriseEditionFeature.Sharing]" :class="$style.content">
					<div>
						...
						/>
						<aura-info-tip v-if="isHomeTeamProject" :bold="false" class="mt-s">
		...
		<template #footer>
			...
			>
				<aura-text v-show="isDirty" color="text-light" size="small" class="mr-xs">
					{{ i18n.baseText('workflows.shareModal.changesHint') }}
				</aura-text>
				<aura-button v-if="isHomeTeamProject" type="secondary" @click="modalBus.emit('close')">
