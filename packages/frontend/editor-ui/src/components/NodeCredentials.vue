...
const selectRefs = ref<Array<InstanceType<typeof auraSelect>>>([]);
					...
					>
						...
						>
							<div :class="[$style.credentialOption, 'mt-2xs', 'mb-2xs']">
								<auraText bold>{{ item.name }}</auraText>
								<auraText size="small">{{ item.typeDisplayName }}</auraText>
							</div>
						</auraOption>
						...
						<template #footer>
							...
							>
								<auraIcon size="xsmall" icon="plus" />
								<auraText bold>{{ NEW_CREDENTIALS_TEXT }}</auraText>
							</div>
						</template>
					</auraSelect>
