...
import { ADD_FORM_NOTICE, getParameterValueByPath, NodeHelpers } from 'aura-workflow';
		...
		>
			...
			<template v-else-if="parameter.type === 'callout'">
				...
				>
					<auraText size="small">
						<auraText
							v-aura-html="i18n.nodeText(activeNode?.type).inputLabelDisplayName(parameter, path)"
							size="small"
						/>
			...
			>
				...
				>
					...
					>
						<auraTooltip>
							...
							<auraIcon icon="triangle-alert" size="small" color="danger" />
						</auraTooltip>
					</template>
				</auraInputLabel>
				<Suspense v-if="!asyncLoadingError">
					...
					<template #fallback>
						<auraText size="small" class="async-notice">
							<auraIcon icon="refresh-cw" size="xsmall" :spin="true" />
							{{ i18n.baseText('parameterInputList.loadingFields') }}
						</auraText>
					</template>
				</Suspense>
				<auraText v-else size="small" color="danger" class="async-notice">
					<auraIcon icon="triangle-alert" size="xsmall" />
