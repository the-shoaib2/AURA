		...
		<template #content>
			<div :class="[$style.descriptionContainer, 'p-s']">
				<div>
					...
					{{ ' ' }}
					<aura-link :to="COMMUNITY_NODES_INSTALLATION_DOCS_URL" @click="onMoreInfoTopClick">
			...
			<div :class="[$style.formContainer, 'mt-m']">
				...
				>
					<aura-text>
						{{ i18n.baseText('settings.communityNodes.installModal.checkbox.label') }} </aura-text
					><br />
					<aura-link :to="COMMUNITY_NODES_RISKS_DOCS_URL" @click="onLearnMoreLinkClick">{{
						i18n.baseText('generic.moreInfo')
					}}</aura-link>
				</el-checkbox>
			</div>
