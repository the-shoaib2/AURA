...
<template>
	<div>
		<aura-loading :loading="loading" :rows="5" variant="p" />
		...
		>
			<aura-tags :tags="categoriesAsTags" @click:tag="redirectToCategory" />
		</TemplateDetailsBlock>
		...
		>
			<div :class="$style.text">
				<aura-text v-if="isTemplatesWorkflow(template)" size="small" color="text-base">
					{{ i18n.baseText('template.details.created') }}
					<TimeAgo :date="template.createdAt" />
					{{ i18n.baseText('template.details.by') }}
					{{ template.user ? template.user.username : 'aura team' }}
				</aura-text>
			</div>
			<div :class="$style.text">
				<aura-text
