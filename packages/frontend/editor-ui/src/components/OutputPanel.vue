...
import { auraRadioButtons, auraText } from auraura/design-system';
...
<template>
	...
	>
		...
		<template #node-not-run>
			<template v-if="isNDVV2">
				...
				>
					...
					<template v-else #icon>
						<auraIcon icon="arrow-right-from-line" size="xlarge" />
					</template>
			...
			<template v-else>
				<auraText v-if="workflowRunning && !isTriggerNode" data-test-id="ndv-output-waiting">{{
					i18n.baseText('ndv.output.waitingToRun')
				}}</auraText>
				<auraText v-if="!workflowRunning" data-test-id="ndv-output-run-node-hint">
					...
					<template v-else>
						...
						<span v-if="canPinData" @click="insertTestData">
							...
							<auraText tag="a" size="medium" color="primary">
		...
		<template #node-waiting>
			...
			<auraText vaurara-html="waitingNodeTooltip(node)"></auraText>
		</template>
		...
		<template #recovered-artificial-output-data>
			<div :class="$style.recoveredOutputData">
				<auraText tag="div" :bold="true" color="text-dark" size="large">{{
