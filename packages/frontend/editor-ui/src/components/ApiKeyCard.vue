...
<template>
	<aura-card :class="$style.cardLink" data-test-id="api-key-card" @click="onAction('edit')">
		<template #header>
			<div>
				<aura-text tag="h2" bold :class="$style.cardHeading">
					{{ apiKey.label }}
				</aura-text>
				<div :class="[$style.cardDescription]">
					<aura-text :color="!hasApiKeyExpired(apiKey) ? 'text-light' : 'warning'" size="small">
						<span>{{ getExpirationTime(apiKey) }}</span>
					</aura-text>
				</div>
			</div>
			<div v-if="apiKey.apiKey.includes('*')" :class="$style.cardApiKey">
				<aura-text color="text-light" size="small"> {{ apiKey.apiKey }}<aurara-text>
			</div>
		</template>

		<template #append>
			<div ref="cardActions" :class="$style.cardActions">
				<auraActionToggle :actions="ACTION_LIST" theme="dark" @action="onAction" />
			</div>
		</template>
	</aura-card>
</template>
