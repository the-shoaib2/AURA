...
import { auraIconButton, auraInfoTip, auraTooltip, auraTree } froaura@aura/design-system';
			...
			<tbody>
				...
				>
					...
					<td
						...
						@mouseleave="onMouseLeaveCell"
					>
						<auraInfoTip>{{ i18n.baseText('runData.emptyItemHint') }}</auraInfoTip>
					</td>
		...
		<table v-else ref="tableRef" :class="$style.table">
			...
			<thead>
				<tr>
					...
					>
						<auraTooltip placement="bottom-start" :disabled="!mappingEnabled" :show-after="1000">
							<template #content>
								<div>
					...
					<th v-if="columnLimitExceeded" :class="$style.header">
						<auraTooltip placement="bottom-end">
							<template #content>
								<div>
									<I18nT
										tag="span"
							...
							<span>
								<aura-icon :class="$style['warningTooltip']" icon="triangle-alert" />
			...
			>
				...
				>
					...
					>
						...
						/>
						<auraTree v-else-if="isObject(data)" :node-class="$style.nodeClass" :value="data">
