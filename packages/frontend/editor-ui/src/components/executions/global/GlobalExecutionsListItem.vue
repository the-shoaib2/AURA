...
<template>
	<tr :class="classes">
		...
		<td>
			<auraTooltip :content="execution.workflowName || workflowName" placement="top">
				<RouterLink
					:to="{
		...
		<td data-test-id="execution-status">
			...
			>
				<div>
					<auraIcon :icon="statusRender.icon" :color="statusRender.color" class="mr-2xs" />
					{{ statusRender.label }}
				</div>
			</GlobalExecutionsListItemQueuedTooltip>
			<auraTooltip
		...
		<td>
			<auraIcon v-if="execution.mode === 'manual'" icon="flask-conical" />
		</td>
		<td>
			<auraButton
		...
		<td>
			<ElDropdown v-if="!isRunning" trigger="click" @command="handleActionItemClick">
				<auraIconButton text type="tertiary" icon="ellipsis-vertical" />
				<template #dropdown>
					<ElDropdownMenu
						:class="{
							[$style.actions]: true,
