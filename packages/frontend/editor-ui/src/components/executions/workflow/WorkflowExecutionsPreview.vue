...
<template>
	<div v-if="executionUIDetails?.name === 'new'" :class="$style.newInfo">
		<auraText :class="$style.newMessage" color="text-light">
			{{ locale.baseText('executionDetails.newMessage') }}
		</auraText>
		<auraButton class="mt-l" type="tertiary" @click="handleStopClick">
	...
	<div v-else-if="executionUIDetails?.name === 'running'" :class="$style.runningInfo">
		...
		<auraText :class="$style.runningMessage" color="text-light">
		...
		>
			<div :class="$style.executionDetailsLeft">
				<div :class="$style.executionTitle">
					<auraText size="large" color="text-dark" :bold="true" data-test-id="execution-time">{{
