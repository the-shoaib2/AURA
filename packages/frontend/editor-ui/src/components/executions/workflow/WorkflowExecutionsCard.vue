	...
	>
		...
		>
			<div :class="$style.description">
				...
				<auraText v-else color="text-dark" :bold="true" size="medium" data-test-id="execution-time">
				...
				<div :class="$style.executionStatus">
					...
					/>
					<auraText :class="$style.statusLabel" size="small">{{ executionUIDetails.label }}</auraText>
					{{ ' ' }}
					<auraText
						v-if="executionUIDetails.name === 'running'"
				...
				<div v-if="execution.mode === 'retry'">
					<auraText :color="isActive ? 'text-dark' : 'text-base'" size="small">
				...
				<div v-if="isAnnotationEnabled" :class="$style.annotation">
					<div v-if="execution.annotation?.vote" :class="$style.ratingIcon">
						<auraIcon v-if="execution.annotation.vote == 'up'" :class="$style.up" icon="thumbs-up" />
						<auraIcon v-else :class="$style.down" icon="thumbs-down" />
					</div>
					<auraTags
			...
			<div :class="$style.icons">
				...
				/>
				<auraTooltip v-if="execution.mode === 'manual'" placement="top">
					<template #content>
						<span>{{ locale.baseText('executionsList.test') }}</span>
					</template>
					<auraIcon :class="[$style.icon, $style.manual]" icon="flask-conical" />
				</auraTooltip>
				<auraTooltip v-if="execution.mode === 'evaluation'" placement="top">
					<template #content>
						<span>{{ locale.baseText('executionsList.evaluation') }}</span>
					</template>
					<auraIcon :class="[$style.icon, $style.evaluation]" icon="check-check" />
				</auraTooltip>
			</div>
		</router-link>
	</div>
