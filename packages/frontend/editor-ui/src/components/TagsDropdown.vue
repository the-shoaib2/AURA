...
import { auraOption, auraSelect } from auraura/design-system';
...
const selectRef = ref<InstanceType<typeof auraSelect>>();
const tagRefs = ref<Array<InstanceType<typeof auraOption>>>();
const createRef = ref<InstanceType<typeof auraOption>>();
	...
	<div ref="container" :class="containerClasses" @keydown.stop>
		...
		>
			<auraOption
				...
				class="ops"
			>
				<aura-icon icon="circle-plus" />
				<span>
					{{ i18n.baseText('tagsDropdown.createTag', { interpolate: { filter } }) }}
				</span>
			</auraOption>
			<auraOption v-else-if="options.length === 0" value="message" disabled>
			...
			<auraOption v-if="manageEnabled" :key="MANAGE_KEY" :value="MANAGE_KEY" class="ops manage-tags">
				<aura-icon icon="cog" />
