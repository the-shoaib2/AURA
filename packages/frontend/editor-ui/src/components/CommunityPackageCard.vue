...
<template>
	<div :class="$style.cardContainer" data-test-id="community-package-card">
		<div v-if="loading" :class="$style.cardSkeleton">
			<aura-loading :class="$style.loader" variant="p" :rows="1" />
			<aura-loading :class="$style.loader" variant="p" :rows="1" />
		</div>
		<div v-else-if="communityPackage" :class="$style.packageCard">
			<div :class="$style.cardInfoContainer">
				<div :class="$style.cardTitle">
					<aura-text :bold="true" size="large">{{ communityPackage.packageName }}<aurara-text>
				</div>
				<div :class="$style.cardSubtitle">
					<aura-text :bold="true" size="small" color="text-light">
						{{
					...
					<aura-text size="small" color="text-light">
			...
			<div :class="$style.cardControlsContainer">
				<aura-text :bold="true" size="large" color="text-light">
					v{{ communityPackage.installedVersion }}
				</aura-text>
				<aura-tooltip v-if="communityPackage.failedLoading === true" placement="top">
					<template #content>
						<div>
							{{ i18n.baseText('settings.communityNodes.failedToLoad.tooltip') }}
						</div>
					</template>
					<aura-icon icon="triangle-alert" color="danger" size="large" />
				</aura-tooltip>
				<aura-tooltip
				...
				>
					...
					<aura-button outline label="Update" @click="onUpdateClick" />
				</aura-tooltip>
				<aura-tooltip v-else placement="top">
					<template #content>
						<div>
							{{ i18n.baseText('settings.communityNodes.upToDate.tooltip') }}
						</div>
					</template>
					<aura-icon icon="circle-check" color="text-light" size="large" />
				</aura-tooltip>
				<div :class="$style.cardActions">
					<aura-action-toggle :actions="packageActions" @action="onAction"><aurara-action-toggle>
				</div>
			</div>
		</div>
	</div>
