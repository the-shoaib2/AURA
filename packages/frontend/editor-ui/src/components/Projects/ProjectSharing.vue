...
<template>
	<div>
		...
		>
			<template #prefix>
				<auraIcon v-if="projectIcon.type === 'icon'" :icon="projectIcon.value" color="text-dark" />
				<auraText v-else-if="projectIcon.type === 'emoji'" color="text-light" :class="$style.emoji">
					{{ projectIcon.value }}
				</auraText>
			</template>
			<auraOption
		...
		<ul v-if="Array.isArray(model)" :class="$style.selectedProjects">
			<li v-if="props.homeProject" :class="$style.project" data-test-id="project-sharing-owner">
				<ProjectSharingInfo :project="props.homeProject">
					<auraBadge theme="tertiary" bold>
						{{ locale.baseText('auth.roles.owner') }}
					</auraBadge></ProjectSharingInfo
				>
			</li>
			<li
			...
			>
				...
				>
					<auraOption v-for="role in roles" :key="role.role" :value="role.role" :label="role.name" />
				</auraSelect>
				<auraButton
					v-if="!props.static"
					type="tertiary"
