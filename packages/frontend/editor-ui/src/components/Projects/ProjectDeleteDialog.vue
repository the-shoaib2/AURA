...
<template>
	<el-dialog
		...
		width="500"
	>
		<aura-text v-if="isCurrentProjectEmpty" color="text-base">{{
		...
		<div v-else>
			...
			<div class="pt-l">
				...
				<div v-if="operation === 'wipe'" :class="$style.operation">
					<aura-input-label :label="locale.baseText('projects.settings.delete.question.wipe.title')">
						<aura-input
		...
		<template #footer>
			<auraButton
				...
				>{{ locale.baseText('projects.settings.danger.deleteProject') }}</auraButton
			>
		</template>
	</el-dialog>
</template>
