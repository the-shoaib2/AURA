...
import { auraButton, auraTooltip } from auraura/design-system';
...
import { type IconOrEmoji, isIconOrEmoji } from '@aura/design-system/components/auraIconPicker/types';
...
const projectDescriptionTruncated = computed(() => {
	...
	// Fallback to auraText component default font-size, small
		...
		<div ref="projectHeaderRef" :class="$style.projectHeader">
			<div :class="$style.projectDetails">
				...
				<div :class="$style.headerActions">
					<auraHeading v-if="projectName" bold tag="h2" size="xlarge" data-test-id="project-name">{{
						projectName
					}}</auraHeading>
					<auraText v-if="sectionDescription" color="text-light" data-test-id="project-subtitle">
						{{ sectionDescription }}
					</auraText>
					<template v-else-if="projectDescription">
						<div :class="$style.projectDescriptionWrapper">
							<auraText color="text-light" data-test-id="project-subtitle">
								{{ projectDescriptionTruncated || projectDescription }}
							</auraText>
							<div v-if="projectDescriptionTruncated" :class="$style.tooltip">
								<auraText color="text-light">{{ projectDescription }}</auraText>
							</div>
						</div>
					</template>
				</div>
