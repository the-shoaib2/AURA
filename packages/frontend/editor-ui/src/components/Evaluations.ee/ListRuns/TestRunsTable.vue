...
import { auraIcon, auraText } from auraura/design-system';
...
<template>
	<div :class="$style.container">
		<auraHeading size="large" :bold="true" :class="$style.runsTableHeading" color="text-base">
		...
		>
			...
			<template #status="{ row }">
				...
				>
					<auraText v-if="row.status === 'running'" color="secondary">
						<AnimatedSpinner />
					</auraText>
					<auraIcon
						v-else
					...
					<template v-if="row.status === 'warning'">
						<auraText color="warning" :class="[$style.alertText, $style.warningText]">
					...
					<template v-else-if="row.status === 'error'">
						<auraTooltip placement="top" :show-after="300">
							...
							<auraText :class="[$style.alertText, $style.errorText]">
								<I18nT :keypath="`${getErrorBaseKey(row.errorCode)}`" scope="global">
									<template
										v-if="
											locale.exists(`${getErrorBaseKey(row.errorCode)}.description` as BaseTextKey)
