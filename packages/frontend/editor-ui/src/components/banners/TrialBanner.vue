...
<template>
	<BaseBanner name="TRIAL" theme="custom">
			...
			<div :class="$style.content">
				...
				<div :class="$style.usageCounter">
					...
					<div :class="$style.executionsCountSection">
						<auraText size="xsmall" :color="trialHasExecutionsLeft ? 'text-dark' : 'danger'">
							{{ currentExecutions }}/{{ maxExecutions }} </auraText
						>&nbsp;<auraText
							size="xsmall"
							:color="trialHasExecutionsLeft ? 'text-dark' : 'danger'"
							>{{ locale.baseText('executionUsage.label.executions') }}</auraText
						>
					</div>
				</div>
			</div>
		...
		<template #trailingContent>
			<aura-button type="success" icon="gem" size="small" @click="onUpdatePlanClick">{{
