...
<template>
	<aura-card :class="$style.cardLink" data-test-id="destination-card" @click="onClick">
		<template #header>
			<div>
				<aura-heading tag="h2" bold :class="$style.cardHeading">
					{{ destination.label }}
				</aura-heading>
				<div :class="$style.cardDescription">
					<aura-text color="text-light" size="small">
		...
		<template #append>
			<div ref="cardActions" :class="$style.cardActions">
				<div :class="$style.activeStatusText" data-test-id="destination-activator-status">
					<aura-text v-if="nodeParameters.enabled" :color="'success'" size="small" bold>
						{{ i18n.baseText('workflowActivator.active') }}
					</aura-text>
					<aura-text v-else color="text-base" size="small" bold>
				...
				<aura-action-toggle :actions="actions" theme="dark" @action="onAction" />
			</div>
		</template>
	</aura-card>
</template>
