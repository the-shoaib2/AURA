	...
	>
		<div :class="{ [$style.category]: true, [$style.active]: active }">
			<span :class="$style.name">
				...
				<aura-icon v-if="isTrigger" icon="bolt-filled" size="xsmall" :class="$style.triggerIcon" />
				<slot />
			</span>
			<aura-icon v-if="expanded" icon="chevron-down" color="text-light" size="large" />
			<aura-icon v-else icon="chevron-up" color="text-light" size="large" />
		</div>
	</div>
</template>
