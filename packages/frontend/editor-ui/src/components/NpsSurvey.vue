	...
	>
		<template #header>
			<div :class="$style.title">
				<aura-heading tag="h2" size="medium" color="text-xlight">{{ modalTitle }}<aurara-heading>
			</div>
		</template>
		<template #content>
			<section :class="$style.content">
				<div v-if="showButtons" :class="$style.wrapper">
					...
					<div :class="$style.text">
						<aura-text size="small" color="text-xlight">{{ NOT_LIKELY_OPTION }}<aurara-text>
						<aura-text size="small" color="text-xlight">{{ VERY_LIKELY_OPTION }}<aurara-text>
					</div>
				</div>
