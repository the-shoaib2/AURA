...
<template>
	...
	>
		<template v-if="selectedGroupIcon" #prefix>
			<aura-icon :class="$style.icon" :icon="selectedGroupIcon" color="text-light" size="small" />
		</template>
					...
					<template #reference>
						...
						>
							<div :class="$style.groupTitle">
								<aura-icon v-if="group.icon" :icon="group.icon" :class="$style.icon" size="small" />
								<span>{{ i18n.baseText(group.name) }}</span>
							</div>
							<aura-icon icon="chevron-right" color="text-light" size="xsmall" />
						</div>
					</template>
					<div>
						<aura-option
