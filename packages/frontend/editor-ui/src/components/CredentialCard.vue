...
<template>
	<aura-card :class="$style.cardLink" @click.stop="onClick">
		<template #prepend>
		...
		<template #header>
			<aura-text tag="h2" bold :class="$style.cardHeading">
				{{ data.name }}
				<auraBadge v-if="readOnly" class="ml-3xs" theme="tertiary" bold>
					{{ locale.baseText('credentials.item.readonly') }}
				</auraBadge>
				<auraBadge v-if="needsSetup" class="ml-3xs" theme="warning">
