...
import { useauraLocalStorage } from '@/composables/useauraLocalStorage';
...
const auraLocalStorage = useauraLocalStorage();
...
<template>
	<PageViewLayout>
		...
		<div v-if="loading" class="resource-list-loading">
			<aura-loading :rows="25" :shrink-last="false" />
		</div>
		<template v-else>
			...
			<PageViewLayoutList v-else>
				<template #header>
					...
					>
						<aura-info-tip :bold="false">
							...
							<aura-link data-test-id="workflows-filter-reset" size="small" @click="resetFilters">
				...
				<div v-if="resourcesRefreshing" class="resource-list-loading resource-list-loading-instant">
					<aura-loading :rows="rowsPerPage" :shrink-last="false" />
				</div>
				<div
