...
import { auraBadge, auraButton, auraLink, auraText } froaura@aura/design-system';
...
<template>
	...
	>
		<template #content>
			<auraText tag="div" class="mb-xs">
				{{ i18n.baseText('settings.sourceControl.modals.pull.description') }}
				<br />
				<auraLink :to="i18n.baseText('settings.sourceControl.docs.using.pushPull.url')">
			...
			<div :class="$style.container">
				...
				>
					<template #default="{ item, index, active }">
						...
						>
							<auraText bold>{{ item.title }}</auraText>
						</div>
						<DynamicScrollerItem
							v-else
							:item="item"
						...
						>
							<div :class="$style.listItem" data-test-id="pull-modal-item">
								...
								<auraText v-else>{{ item.name }}</auraText>
								<auraBadge :theme="getStatusTheme(item.status)" :class="$style.listBadge">
		...
		<template #footer>
			<div :class="$style.footer">
				<auraButton type="tertiary" class="mr-2xs" @click="close">
					{{ i18n.baseText('settings.sourceControl.modals.pull.buttons.cancel') }}
				</auraButton>
				<auraButton type="primary" data-test-id="force-pull" @click="pullWorkfolder">
