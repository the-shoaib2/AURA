...
<template>
	<aura-card :class="$style.card">
		<div :class="$style.cardBody">
			<ExternalSecretsProviderImage :class="$style.cardImage" :provider="provider" />
			<div :class="$style.cardContent">
				<aura-text bold>{{ provider.displayName }}<aurara-text>
				<aura-text v-if="provider.connected" color="text-light" size="small">
					<span>
						{{
			...
			<div v-if="provider.name === 'infisical'" :class="$style.deprecationWarning">
				<aura-icon :class="$style['warningTriangle']" icon="triangle-alert" />
				<auraBadge class="mr-xs" theme="tertiary" bold data-test-id="card-badge">
			...
			<aura-button v-else type="tertiary" @click="openExternalSecretProvider()">
