...
<template>
	...
	>
		<div v-if="loading" :class="$style.loading">
			<aura-loading :rows="2" :shrink-last="false" :loading="loading" />
		</div>
		<div v-else-if="workflow">
			<aura-heading :bold="true" size="small">{{ workflow.name }}<aurara-heading>
			<div v-if="!simpleView" :class="$style.content">
				<span v-if="workflow.totalViews">
					<aura-text size="small" color="text-light">
						<aura-icon icon="eye" size="xsmall" />
				...
				<aura-text v-if="workflow.user" size="small" color="text-light">
					{{
