...
import { auraNavigationDropdown, auraTooltip, auraLink, auraIconButton } froaura@aura/design-system';
	...
	},
	{
		...
		children: [
			...
			},
			{
				...
				link: {
					href: 'https://docs.aura.io?utm_sourceaurara_app&utm_medium=app_sidebar',
					target: '_blank',
				},
			},
			{
				...
				link: {
					href: 'https://community.aura.io?utm_sourceaurara_app&utm_medium=app_sidebar',
					target: '_blank',
				},
			},
			{
				...
				link: {
					href: 'https://docs.aura.io/courses/',
					target: '_blank',
				},
			},
			{
			...
			},
			{
				id: 'about',
				icon: 'info',
				label: i18n.baseText('mainSidebar.aboutaura'),
				position: 'bottom',
			},
		],
	},
...
const createBtn = ref<InstanceType<typeof auraNavigationDropdown>>();
...
<template>
	...
	>
		...
		>
			<auraIcon v-if="isCollapsed" icon="chevron-right" size="xsmall" class="ml-5xs" />
			<auraIcon v-else icon="chevron-left" size="xsmall" class="mr-5xs" />
		</div>
		<div :class="$style.logo">
				...
				>
					<template #content>
						<I18nT keypath="readOnlyEnv.tooltip" scope="global">
							<template #link>
								<auraLink
									to="https://docs.aura.io/source-control-environments/setup/#step-4-connectaurara-and-configure-your-instance"
									size="small"
								>
			...
			<auraNavigationDropdown
				ref="createBtn"
				data-test-id="universal-add"
				:menu="menu"
				@select="handleMenuSelect"
			>
				<auraIconButton icon="plus" type="secondary" outline />
				<template #[createWorkflowsAppendSlotName]>
					...
					>
						<auraIcon style="margin-left: auto; margin-right: 5px" icon="lock" size="xsmall" />
					</auraTooltip>
				</template>
		...
		<auraMenu :items="mainMenuItems" :collapsed="isCollapsed" @select="handleSelect">
			<template #header>
				<ProjectNavigation
					:collapsed="isCollapsed"
			...
			<template v-if="showUserArea" #footer>
				<div ref="user" :class="$style.userArea">
					...
					>
						<auraText size="small" :bold="true" color="text-dark">{{
