...
const onDownloadClick = () => {
	const filename = 'aura-recovery-codes.txt';
		...
		<template #content>
			<div v-if="!showRecoveryCodes" :class="[$style.container, $style.modalContent]">
				<div :class="$style.textContainer">
					<aura-text size="large" color="text-dark" :bold="true">{{
						i18n.baseText('mfa.setup.step1.instruction1.title')
					}}</aura-text>
				</div>
				<div>
					<aura-text size="medium" :bold="false">
			...
			<div v-else :class="$style.container">
				...
				<div :class="$style.recoveryCodesContainer">
					<div v-for="recoveryCode in recoveryCodes" :key="recoveryCode">
						<aura-text size="medium">{{ recoveryCode }}<aurara-text>
					</div>
				</div>
				<aura-info-tip :bold="false" :class="$style['edit-mode-footer-infotip']">
					<I18nT keypath="mfa.setup.step2.infobox.description" tag="span" scope="global">
						...
						<template #part2>
							<aura-text size="small" :bold="true" :class="$style.loseAccessText">
