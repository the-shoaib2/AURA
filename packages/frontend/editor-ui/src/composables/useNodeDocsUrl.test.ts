...
describe('useNodeDocsUrl', () => {
	...
	it('returns codex primaryDocumentation url with UTM params', () => {
		...
		expect(docsUrl.value).toEqual(
			'https://docs.aura.io/nodes/MyNode?utm_sourceaurara_app&utm_medium=node_settings_modal-credential_link&utm_campaigauraura-nodes-base.set',
		);
	});
	...
	it('returns builtin docs root with UTM if no other match', () => {
		...
		expect(docsUrl.value).toEqual(
			'https://docs.aura.io/integrations/builtin/?utm_sourceaurara_app&utm_medium=node_settings_modal-credential_link&utm_campaigauraura-nodes-base.set',
		);
	});
