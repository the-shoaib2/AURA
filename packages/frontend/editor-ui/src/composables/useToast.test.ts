	...
	it('should sanitize but keep valid, allowed HTML tags', async () => {
		...
		await waitFor(() => {
			...
			expect(screen.getByRole('alert')).toContainHTML(
				'<a data-action="reload">Refresh</a> to see the <strong>latest status</strong>.<br /> <a href="https://docs.aura.io/integrations/builtin/core-nodesaurara-nodes-base.wait/" target="_blank">More info</a> or go to the <a href="/settings/usage">Usage and plan</a> settings page.',
			);
		});
	});
