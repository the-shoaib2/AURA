...
<template>
	<div :class="$style.container" data-test-id="personal-settings-container">
		<div :class="$style.header">
			...
			<div v-if="currentUser" :class="$style.user">
				<span :class="$style.username" data-test-id="current-user-name">
					<aura-text color="text-base" bold>{{ currentUser.fullName }}<aurara-text>
					<auraTooltip placement="bottom">
						<template #content>{{ currentUserRole.description }}</template>
						<aura-text :class="$style.tooltip" color="text-light" data-test-id="current-user-role">{{
		...
		<div v-if="isPersonalSecurityEnabled">
			<div class="mb-s">
				<aura-heading size="large">{{ i18n.baseText('settings.personal.security') }}<aurara-heading>
			</div>
			<div class="mb-s">
				<aura-input-label :label="i18n.baseText('auth.password')">
					<aura-link data-test-id="change-password-link" @click="openPasswordModal">{{
			...
			<div v-if="isMfaFeatureEnabled" data-test-id="mfa-section">
				<div class="mb-xs">
					<aura-input-label :label="i18n.baseText('settings.personal.mfa.section.title')" />
					<aura-text :bold="false" :class="$style.infoText">
						{{
							mfaDisabled
								? i18n.baseText('settings.personal.mfa.button.disabled.infobox')
								: i18n.baseText('settings.personal.mfa.button.enabled.infobox')
						}}
						<aura-link :to="MFA_DOCS_URL" size="small" :bold="true">
		...
		<div>
			...
			<div>
				<aura-input-label :label="i18n.baseText('settings.personal.theme')">
					<aura-select
