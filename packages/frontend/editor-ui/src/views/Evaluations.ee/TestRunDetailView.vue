	...
	<div :class="$style.container" data-test-id="test-definition-run-detail">
		<div :class="$style.header">
			<button :class="$style.backButton" @click="router.back()">
				<auraIcon icon="arrow-left" />
				<aura-heading size="large" :bold="true">{{
		...
		<aura-callout v-if="run?.status === 'error'" theme="danger" icon="triangle-alert" class="mb-s">
			<auraText size="small" :class="$style.capitalized">
				{{
					locale.baseText(
		...
		<el-scrollbar always :class="$style.scrollableSummary" class="mb-m">
			<div style="display: flex">
				<div :class="$style.summaryCard">
					<auraText size="small" :class="$style.summaryCardTitle">
						{{ locale.baseText('evaluation.runDetail.totalCases') }}
					</auraText>
					<auraText size="xlarge" :class="$style.summaryCardContentLargeNumber" bold>{{
						testCases.length
					}}</auraText>
				</div>
				...
				<div :class="$style.summaryCard">
					...
					<div>
						<auraText size="medium"> {{ formattedTime.date }} {{ formattedTime.time }} </auraText>
					</div>
				</div>
				...
				<div v-for="(value, key) in metrics" :key="key" :class="$style.summaryCard">
					<auraTooltip :content="key" placement="top">
						<auraText
							size="small"
		...
		<div v-if="isLoading" :class="$style.loading">
			<aura-loading :loading="true" :rows="5" />
		</div>
			...
			<template #status="{ row }">
				<div style="display: inline-flex; gap: 12px; align-items: center; max-width: 100%">
					...
					<template v-if="row.status === 'error'">
						<auraTooltip placement="top" :show-after="300">
							<template #content>
								{{
									locale.baseText(`${getErrorBaseKey(row.errorCode)}` as BaseTextKey) || row.status
								}}
							</template>
							<auraText color="danger" :class="$style.capitalized">
								{{
									locale.baseText(`${getErrorBaseKey(row.errorCode)}` as BaseTextKey) || row.status
								}}
							</auraText>
					...
					<template v-else>
						<auraText :class="$style.capitalized">
							{{ row.status }}
						</auraText>
					</template>
				</div>
