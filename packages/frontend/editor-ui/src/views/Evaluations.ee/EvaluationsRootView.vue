	...
	<div :class="$style.evaluationsView">
		<template v-if="isReady && showWizard">
			<div :class="$style.setupContent">
				<div>
					<auraText size="large" color="text-dark" tag="h3" bold>
						{{ locale.baseText('evaluations.setupWizard.title') }}
					</auraText>
					<auraText tag="p" size="small" color="text-base" :class="$style.description">
						{{ locale.baseText('evaluations.setupWizard.description') }}
						<auraLink size="small" href="https://docsaurara.io/advanced-ai/evaluations/overview">{{
				...
				<div :class="$style.config">
					<iframe
						...
						title="aura Evaluation quickstart"
						frameborder="0"
