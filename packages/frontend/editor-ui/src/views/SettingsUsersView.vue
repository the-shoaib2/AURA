...
import type { TableOptions } from '@aura/design-system/components/auraDataTableServer';
import {
...
<template>
	<div :class="$style.container">
		<auraHeading tag="h1" size="2xlarge" class="mb-xl">
			{{ i18n.baseText('settings.users') }}
			<auraText v-if="!showUMSetupWarning" :class="$style.userCount" color="text-light">{{
		...
		<auraNotice v-if="!isAdvancedPermissionsEnabled">
		...
		<div :class="$style.settingsContainer">
			<div :class="$style.settingsContainerInfo">
				<auraText :bold="true"
					...
					<auraBadge v-if="!isEnforceMFAEnabled" class="ml-4xs">{{
						i18n.baseText('generic.upgrade')
					}}</auraBadge>
				</auraText>
				<auraText size="small" color="text-light">{{
		...
		<div v-if="!showUMSetupWarning" :class="$style.buttonContainer">
			...
			<auraTooltip :disabled="!ssoStore.isSamlLoginEnabled">
				<template #content>
