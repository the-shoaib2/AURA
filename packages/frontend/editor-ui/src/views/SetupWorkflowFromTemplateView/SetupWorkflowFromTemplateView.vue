...
import auraHeading from '@aura/design-system/components/auraHeading';
import auraLink from 'aurara/design-system/components/auraLink';
...
<template>
	<TemplatesView :go-back-enabled="true">
		<template #header>
			<auraHeading v-if="isReady" tag="h1" size="2xlarge"
				>{{ i18n.baseText('templateSetup.title', { interpolate: { name: title } }) }}
			</auraHeading>
			<aura-loading v-else variant="h1" />
		</template>
			...
			<div :class="$style.grid">
				...
				<div>
					...
					<div v-else :class="$style.appCredentialsContainer">
						<aura-loading :class="$style.appCredential" variant="p" :rows="3" />
						<aura-loading :class="$style.appCredential" variant="p" :rows="3" />
					</div>
				</div>

				<div :class="$style.actions">
					<auraLink :href="skipSetupUrl" :new-window="false" @click="onSkipSetup($event)">{{
