...
<template>
	<div>
		<aura-heading size="2xlarge" tag="h1">{{
		...
		>
			<aura-callout theme="secondary" icon="info" class="mt-2xl mb-l">
			...
			<aura-heading size="xlarge" tag="h2" class="mb-s">{{
			...
			<div :class="$style.group">
				...
				<div :class="$style.groupFlex">
					...
					<aura-button
						...
						@click="onDisconnect"
						>{{ locale.baseText('settings.sourceControl.button.disconnect') }}</aura-button
					>
				</div>
			</div>
			<div v-if="sourceControlStore.preferences.publicKey" :class="$style.group">
				...
				<aura-notice type="info" class="mt-s">
			...
			<aura-button
				...
				@click="onConnect"
				>{{ locale.baseText('settings.sourceControl.button.connect') }}</aura-button
			>
			<div v-if="isConnected" data-test-id="source-control-connected-content">
				...
				<div :class="$style.group">
					...
					<div>
						<aura-color-picker v-model="sourceControlStore.preferences.branchColor" size="small" />
					</div>
				</div>
				<div :class="[$style.group, 'pt-s']">
					<aura-button
						...
						@click="onSave"
						>{{ locale.baseText('settings.sourceControl.button.save') }}</aura-button
					>
				</div>
			</div>
		</div>
