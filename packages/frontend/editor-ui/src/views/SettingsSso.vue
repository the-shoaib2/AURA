...
<template>
	<div class="pb-2xl">
		<div :class="$style.heading">
			<aura-heading size="2xlarge">{{ i18n.baseText('settings.sso.title') }}<aurara-heading>
		</div>
		<aura-info-tip>
			{{ i18n.baseText('settings.sso.info') }}
			<a href="https://docs.aura.io/user-management/saml/" target="_blank">
			...
			<div v-if="ssoStore.isEnterpriseSamlEnabled" data-test-id="sso-content-licensed">
				...
				<div :class="$style.group">
					...
					<div class="mt-2xs mb-s">
						<aura-radio-buttons v-model="ipsType" :options="ipsOptions" />
					</div>
