		...
		<template #header>
			<div v-if="!notFoundError" :class="$style.wrapper">
				<div :class="$style.title">
					<aura-heading v-if="template && template.name" tag="h1" size="2xlarge">{{
						template.name
					}}</aura-heading>
					<aura-text v-if="template && template.name" color="text-base" size="small">
						{{ i18n.baseText('generic.workflow') }}
					</aura-text>
					<aura-loading :loading="!template || !template.name" :rows="2" variant="h1" />
				</div>
				<div :class="$style.button">
					...
					/>
					<aura-loading :loading="!template" :rows="1" variant="button" />
				</div>
			</div>
			<div v-else :class="$style.notFound">
				<aura-text color="text-base">{{ i18n.baseText('templates.workflowsNotFound') }}<aurara-text>
			</div>
		</template>
