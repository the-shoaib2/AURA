...
import auraInfoTip from 'aurara/design-system/components/auraInfoTip';
...
<template>
	<div class="settings-usage-and-plan">
		<aura-heading tag="h2" size="2xlarge">{{
			locale.baseText('settings.usageAndPlan.title')
		}}</aura-heading>
		<div v-if="!usageStore.isLoading">
			<aura-heading tag="h3" :class="$style.title" size="large">
				...
				<span v-if="badgedPlanName.badge && badgedPlanName.name" :class="$style.titleTooltip">
					<auraTooltip placement="top">
						...
						<auraBadge>{{ badgedPlanName.badge }}</auraBadge>
					</auraTooltip>
				</span>
			</aura-heading>

			<auraNotice v-if="isCommunity && canUserRegisterCommunityPlus" class="mt-0" theme="warning">
			...
			<div :class="$style.quota">
				<aura-text size="medium" color="text-light">
			...
			<auraInfoTip>{{ locale.baseText('settings.usageAndPlan.activeWorkflows.hint') }}</auraInfoTip>
			...
			<div :class="$style.buttons">
				...
				<aura-button v-if="usageStore.managementToken" size="large" @click="onManagePlan">
					<a :href="managePlanUrl" target="_blank">{{
						locale.baseText('settings.usageAndPlan.button.manage')
					}}</a>
				</aura-button>
				<aura-button v-else size="large" @click.prevent="onViewPlans">
			...
			>
				...
				<template #footer>
					<aura-button type="secondary" @click="activationKeyModal = false">
						{{ locale.baseText('settings.usageAndPlan.dialog.activation.cancel') }}
					</aura-button>
					<aura-button @click="onLicenseActivation">
