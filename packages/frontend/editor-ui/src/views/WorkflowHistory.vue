...
<template>
	<div :class="$style.view">
		<div :class="$style.header">
			<aura-heading tag="h2" size="medium">
				{{ activeWorkflow?.name }}
			</aura-heading>
			<span v-if="activeWorkflow?.isArchived">
				<auraBadge class="ml-s" theme="tertiary" bold data-test-id="workflow-archived-tag">
					{{ i18n.baseText('workflows.item.archived') }}
				</auraBadge>
			</span>
		</div>
		<div :class="$style.corner">
			<aura-heading tag="h2" size="medium" bold>
				{{ i18n.baseText('workflowHistory.title') }}
			</aura-heading>
			<router-link :to="editorRoute" data-test-id="workflow-history-close-button">
				<aura-button type="tertiary" icon="x" size="small" text square />
			</router-link>
		</div>
