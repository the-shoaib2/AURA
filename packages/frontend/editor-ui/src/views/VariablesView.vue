...
import { uid } from '@aura/design-system/utils';
...
<template>
	...
	>
		<template #header>
			<aura-heading size="2xlarge" class="mb-m">
				{{ i18n.baseText('variables.heading') }}
			</aura-heading>
		</template>
		<template #add-button>
			<auraTooltip placement="top" :disabled="canCreateVariables">
				<div>
					<auraButton
						size="medium"
						block
		...
		<template v-if="!isFeatureEnabled" #preamble>
			<auraActionBox class="mb-m" v-bind="unavailableNoticeProps" />
		</template>
		<template v-if="!isFeatureEnabled || (isFeatureEnabled && !canCreateVariables)" #empty>
			<auraActionBox v-if="!isFeatureEnabled" v-bind="unavailableNoticeProps" />
			<auraActionBox
		...
		<template #default="{ data }">
			...
			<tr v-else data-test-id="variables-row">
				...
				<td>
					...
					<auraBadge v-else theme="warning"> Value missing </auraBadge>
				</td>
				<td>
				...
				<td v-if="isFeatureEnabled" align="right">
					<div class="action-buttons">
						<auraTooltip :disabled="permissions.update" placement="top">
							<auraButton
						...
						<auraTooltip :disabled="permissions.delete" placement="top">
							<auraButton
