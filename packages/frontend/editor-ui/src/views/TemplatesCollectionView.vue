		...
		<template #header>
			<div v-if="!notFoundError" :class="$style.wrapper">
				<div :class="$style.title">
					<aura-heading v-if="collection && collection.name" tag="h1" size="2xlarge">
						{{ collection.name }}
					</aura-heading>
					<aura-text v-if="collection && collection.name" color="text-base" size="small">
						{{ i18n.baseText('templates.collection') }}
					</aura-text>
					<aura-loading :loading="!collection || !collection.name" :rows="2" variant="h1" />
				</div>
			</div>
			<div v-else :class="$style.notFound">
				<aura-text color="text-base">{{ i18n.baseText('templates.collectionsNotFound') }}<aurara-text>
			</div>
		</template>
