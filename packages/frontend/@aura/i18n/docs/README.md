...
aura allows for internalization of the majority of UI text:
...
Currently, aura does _not_ allow for internalization of:
...
By default, aura runs in the `en` (English) locale. To have run it in a different locale, set the `aura_DEFAULT_LOCALE` environment variable to a locale identifier. When running in a non-`en` locale, aura will display UI strings for the selected locale and fall back to `en` for any untranslated strings.

```
export aura_DEFAULT_LOCALE=de
pnpm start
```
...
```
Initializing aura process
aura ready on 0.0.0.0, port 5678
Version: 0.156.0
Locale: de
...
```json
{
	...
	"nodeView.owner.placeholder": "🇩🇪 the-shoaib2",
...
pnpm aura-generate-translations
pnpm watch
```
...
If a `headerText` section was changed, re-run `pnpm aura-generate-translations` in `/nodes-base`.
