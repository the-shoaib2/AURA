import type { auraLocale, auraLocaleTranslateFn } from '@aura/design-system/types';
...
let i18nHandler: auraLocaleTranslateFn;
...
export async function use(l: string) {
	try {
		const ndsLang = (await import(`./lang/${l}.ts`)) as { default: auraLocale };
...
export function i18n(fn: auraLocaleTranslateFn) {
