...
import { isSupportedIconName } from '../auraIcon/icons';
...
const auraNodeIcon = getCurrentInstance()?.type;
</script>

<template>
	<div class="aura-node-icon" v-bind="$attrs">
		<div
			:class="{
		...
		>
			...
			<auraTooltip v-if="showTooltip" :placement="tooltipPosition" :disabled="!showTooltip">
				<template #content>
					{{ nodeTypeName }}
				</template>
			...
			<template v-else>
				<div v-if="type !== 'unknown'" :class="$style.icon">
					...
					<div v-if="badge" :class="$style.badge" :style="badgeStyleData">
						<auraNodeIcon :type="badge.type" :src="badge.src" :size="badgeSize" />
					</div>
				</div>
				<div v-else :class="$style.nodeIconPlaceholder">
					{{ nodeTypeName ? nodeTypeName.charAt(0) : '?' }}
