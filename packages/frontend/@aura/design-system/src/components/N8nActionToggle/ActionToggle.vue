...
import type { IconOrientation, IconSize } from '@aura/design-system/types/icon';
...
defineOptions({ name: 'auraActionToggle' });
			...
			<template #dropdown>
				...
				>
					<ElDropdownItem v-for="index in loadingRowCount" :key="index" :disabled="true">
						<template #default>
							<auraLoading :class="$style.loading" animated variant="text" />
						</template>
					</ElDropdownItem>
				</ElDropdownMenu>
