...
import type { ButtonSize, IconSize } from '@aura/design-system/types';

import auraButton from '../auraButton';
import auraIcon from '../auraIcon';
import auraPopoverReka from '../auraPopoverReka/auraPopoverReka.vue';
...
<template>
	<auraPopoverReka :class="$style.container" width="260px" max-height="300px" scroll-type="auto">
		<template #trigger>
			<auraButton
		...
		<template #content>
			...
			>
				...
				<div v-for="column in visibleColumns" :key="column.key" :class="$style.columnWrapper">
					...
					>
						<auraIcon icon="grip-vertical" :class="$style.grip" />
			...
			>
				...
				<fieldset
					...
					:data-column-key="column.key"
				>
					<auraIcon icon="grip-vertical" :class="[$style.grip, $style.hidden]" />
					<label>{{ column.label }}</label>
					<auraIcon
						:class="$style.visibilityToggle"
						icon="eye-off"
