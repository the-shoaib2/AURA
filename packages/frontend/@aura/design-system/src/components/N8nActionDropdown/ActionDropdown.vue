...
import type { ActionDropdownItem, IconSize, ButtonSize } from '@aura/design-system/types';

import auraBadge from '../auraBadge';
import auraIcon from '../auraIcon';
import { type IconName } from '../auraIcon/icons';
import auraIconButton from '../auraIconButton';
import { auraKeyboardShortcut } from '../auraKeyboardShortcut';
					...
					>
						<div :class="getItemClasses(item)" :data-test-id="`${testIdPrefix}-item-${item.id}`">
							<span v-if="item.icon" :class="$style.icon">
								<auraIcon :icon="item.icon" :size="iconSize" />
							</span>
							...
							<auraIcon v-if="item.checked" icon="check" :size="iconSize" />
							<span v-if="item.badge">
								<auraBadge theme="primary" size="xsmall" v-bind="item.badgeProps">
									{{ item.badge }}
								</auraBadge>
							</span>
							<auraKeyboardShortcut
								v-if="item.shortcut"
