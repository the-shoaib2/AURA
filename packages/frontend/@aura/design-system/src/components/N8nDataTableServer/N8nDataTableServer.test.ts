...
import auraDataTableServer, { type TableHeader } from './auraDataTableServer.vue';

const renderComponent = createComponentRenderer(auraDataTableServer);
...
describe('auraDataTableServer', () => {
	it('should render a table', () => {
		const { container } = render(auraDataTableServer, {
	...
	it('should synchronize the state', async () => {
		const { container, rerender } = render(auraDataTableServer, {
			//@ts-expect-error testing-library errors due to header generics
			props: { items, headers, itemsLength: items.length },
		});
	...
	it('should not show the pagination if there are no items', async () => {
		const { queryByTestId } = render(auraDataTableServer, {
			//@ts-expect-error testing-library errors due to header generics
