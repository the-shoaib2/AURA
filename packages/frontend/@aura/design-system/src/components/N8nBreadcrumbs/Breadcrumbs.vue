...
import auraActionToggle from '../auraActionToggle';
...
defineOptions({ name: 'auraBreadcrumbs' });
		...
		<ul :class="$style.list">
			...
			>
				...
				<div v-if="props.theme !== 'small'" :class="$style['hidden-items-menu']">
					...
					>
						<auraText :bold="true" :class="$style.dots">...</auraText>
					</auraActionToggle>
				</div>
					...
					<template #content>
						...
						<div v-else :class="$style.tooltipContent">
							<div data-test-id="hidden-items-tooltip">
								<auraText>{{ loadedHiddenItems.map((item) => item.label).join(' / ') }}</auraText>
							</div>
						</div>
					</template>
			...
			<template v-for="(item, index) in items" :key="item.id">
				...
				>
					<auraLink v-if="item.href" :href="item.href" theme="text">{{ item.label }}</auraLink>
					<auraText v-else>{{ item.label }}</auraText>
				</li>
