...
// Simple mock for the auraActionToggle component to test hidden items and events
const auraActionToggleMock = defineComponent({
	name: 'auraActionToggle',
	props: {
		actions: {
...
describe('Breadcrumbs', async () => {
	...
	it('renders hidden items correctly', async () => {
		...
		const { container, getByTestId, queryByTestId } = render(Breadcrumbs, {
			...
			global: {
				stubs: {
					'aura-action-toggle': auraActionToggleMock,
				},
				plugins: [router],
			},
		});
