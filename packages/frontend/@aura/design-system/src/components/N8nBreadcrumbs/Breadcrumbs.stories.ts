...
import ActionToggle from '../auraActionToggle/ActionToggle.vue';
import Tags from '../auraTags/Tags.vue';
...
const withSlotsTemplate: StoryFn = (args, { argTypes }) => ({
    ...
    <template #prepend>
      <div style="display: flex; align-items: center; gap: 8px;">
        <aura-icon icon="layers"/>
        <aura-text>My Project</aura-text>
      </div>
    </template>
    <template #append>
      <div style="display: flex; align-items: center;">
				<aura-tags :tags="testTags" />
        <aura-action-toggle size="small" :actions="testActions" theme="dark"/>
      </div>
    </template>
  </Breadcrumbs>`,
});
