...
<template>
	<div :class="$style.container" data-test-id="code-diff-suggestion">
		...
		<div :class="$style.actions">
			<div v-if="error">
				<auraIcon icon="triangle-alert" color="danger" class="mr-5xs" />
			...
			<div v-else-if="replaced">
				...
				<auraIcon icon="check" color="success" class="ml-xs" />
			...
			<auraButton
				...
				:loading="replacing"
				@click="() => emit('replace')"
				>{{ replacing ? t('codeDiff.replacing') : t('codeDiff.replaceMyCode') }}</auraButton
			>
		</div>
	</div>
</template>
