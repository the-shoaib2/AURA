...
import type { ButtonProps } from '@aura/design-system/types/button';
...
watchEffect(() => {
	if (props.element === 'a' && !props.href) {
		console.error('aura-button:href is required for link buttons');
	}
});
...
<template>
	...
	>
		<span v-if="loading || icon" :class="$style.icon">
			<auraSpinner v-if="loading" :size="iconSize" />
			<auraIcon v-else-if="icon" :icon="icon" :size="iconSize" />
		</span>
...
.success {
	@include aura-button-success;
}

.warning {
	@include aura-button-warning;
}

.danger {
	@include aura-button-danger;
}
