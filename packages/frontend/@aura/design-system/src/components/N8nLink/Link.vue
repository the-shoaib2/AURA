...
import type { TextSize } from '@aura/design-system/types/text';

import auraRoute from '../auraRoute';
...
<template>
	<auraRoute :to="to" :new-window="newWindow" v-bind="$attrs" class="aura-link">
		<span :class="$style[`${underline ? `${theme}-underline` : theme}`]">
			<auraText :size="size" :bold="bold">
				<slot></slot>
			</auraText>
		</span>
	</auraRoute>
