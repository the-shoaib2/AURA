...
<template>
	...
	>
		<div v-if="scrollPosition > 0" :class="$style.back" @click="scrollLeft">
			<auraIcon icon="chevron-left" size="small" />
		</div>
		<div v-if="canScrollRight" :class="$style.next" @click="scrollRight">
			<auraIcon icon="chevron-right" size="small" />
		</div>
		<div ref="tabs" :class="$style.tabs">
			...
			>
				<auraTooltip :disabled="!option.tooltip" placement="bottom" :show-after="100">
					<template #content>
						<div v-aura-html="option.tooltip" @click="handleTooltipClick(option.value, $event)" />
					</template>
					<a
						v-if="option.href"
						target="_blank"
					...
					>
						<auraIcon v-if="option.icon" :icon="option.icon" size="medium" />
