...
import type { IconSize, IconColor } from '@aura/design-system/types/icon';
...
const classes = computed(() => {
	...
	return ['aura-icon', ...applied.map((c) => $style[c])];
});
...
const styles = computed(() => {
	...
	if (props.strokeWidth) {
		stylesToApply['--aura-icon-stroke-width'] = `${props.strokeWidth}px`;
	}
...
.strokeWidth {
	rect,
	path {
		stroke-width: var(--aura-icon-stroke-width);
	}
}
