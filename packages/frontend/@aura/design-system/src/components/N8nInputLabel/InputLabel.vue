...
defineOptions({ name: 'auraInputLabel' });
			...
			>
				<div :class="$style['main-content']">
					<div v-if="label" :class="$style.title">
						...
						>
							{{ label }}
							<auraText v-if="required" color="primary" :bold="bold" :size="size">*</auraText>
						</auraText>
					</div>
					<span
					...
					>
						<auraTooltip placement="top" :popper-class="$style.tooltipPopper" :show-after="300">
							<auraIcon :class="$style.icon" icon="circle-help" size="small" />
							<template #content>
								<div v-aura-html="addTargetBlank(tooltipText)" />
							</template>
						</auraTooltip>
					</span>
				</div>
