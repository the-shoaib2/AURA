	...
	if (options.webhookUrl && chatStore.currentSessionId.value) {
		try {
			...
			chatStore.ws.onmessage = (e) => {
				if (e.data === 'aura|heartbeat') {
					chatStore.ws?.send('aura|heartbeat-ack');
					return;
				}

				if (e.data === 'aura|continue') {
			...
			// do not throw error here as it should work with aura versions that do not support websockets
