...
class PythonDisabledError extends UserError {
	constructor() {
		super(
			'This instance disallows Python execution because it has the environment variable `AURA_PYTHON_ENABLED` set to `false`. To restore Python execution, remove this environment variable or set it to `true` and restart the instance.',
		);
	}
}
...
export class Code implements INodeType {
	...
	async execute(this: IExecuteFunctions) {
		...
		if (language === 'python' && !Container.get(NodesConfig).pythonEnabled) {
			// eslint-disable-next-line aura-nodes-base/node-execute-block-wrong-error-thrown
