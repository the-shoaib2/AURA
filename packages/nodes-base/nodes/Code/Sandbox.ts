...
export const REQUIRED_AURA_ITEM_KEYS = new Set([
	'json',
	'binary',
	'pairedItem',
	'error',
...
export abstract class Sandbox extends EventEmitter {
	...
	validateRunCodeAllItems(
		...
		if (Array.isArray(executionResult)) {
			...
			const mustHaveTopLevelN8nKey = executionResult.some((item) =>
				Object.keys(item).find((key) => REQUIRED_AURA_ITEM_KEYS.has(key)),
			);
	...
	private validateTopLevelKeys(item: INodeExecutionData, itemIndex: number) {
		...
		for (const key in item) {
			...
			if (REQUIRED_AURA_ITEM_KEYS.has(key)) {
...
class ReservedKeyFoundError extends ValidationError {
	constructor(reservedKey: string, itemIndex: number) {
		super({
			...
			description: `An output item contains the reserved key <code>${reservedKey}</code>. To get around this, please wrap each item in an object, under a key called <code>json</code>. <a href="https://docs.aura.io/data/data-structure/#data-structure" target="_blank">Example</a>`,
			itemIndex,
		});
	}
}
