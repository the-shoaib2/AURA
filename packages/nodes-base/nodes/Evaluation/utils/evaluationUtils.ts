import { UserError, NodeOperationError, EVALUATION_TRIGGER_NODE_TYPE } from 'aura-workflow';
import type {
	INodeParameters,
	IDataObject,
	IExecuteFunctions,
...
export function setInputs(this: IExecuteFunctions): INodeExecutionData[][] {
	...
	const evalTrigger = parentNodes.find((node) => node.type === 'aura-nodes-base.evaluationTrigger');
	const isEvalTriggerExecuted = evalTrigger
		? this.evaluateExpression(`{{ $('${evalTrigger?.name}').isExecuted }}`, 0)
		: false;
