...
export class E2EController {
	...
	async setEnvFeatureFlags(req: Request<{}, {}, { flags: Record<string, string> }>) {
		...
		// Validate that all flags start with aura_ENV_FEAT_
		for (const key of Object.keys(flags)) {
			if (!key.startsWith('AURAa_ENV_FEAT_')) {
				return {
					success: false,
					message: `Invalid flag key: ${key}. Must start with AURAa_ENV_FEAT_`,
				};
			}
		}

		// Clear existing AURAa_ENV_FEAT_ environment variables
