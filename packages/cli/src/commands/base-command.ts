...
import { aura_VERSION, aura_RELEASE_DATE } from '@/constants';
...
export abstract class BaseCommand<F = never> {
	...
	async init(): Promise<void> {
		...
		await this.errorReporter.init({
			...
			releaseDate: AURAa_RELEASE_DATE,
		});
	...
	async initObjectStoreService() {
		...
		if (isSelected && !isAvailable) {
			throw new UserError(
				'External storage selected but unavailable. Please make external storage available by adding "s3" to `AURAa_AVAILABLE_BINARY_DATA_MODES`.',
			);
		}
		...
		if (!isLicensed) {
			this.logger.error(
				'No license found for S3 storage. \n Either set `AURAa_DEFAULT_BINARY_DATA_MODE` to something else, or upgrade to a license that supports this feature.',
			);
