...
const flagsSchema = z.object({
	...
	reinstallMissingPackages: z
		.boolean()
		.describe(
			'Attempts to self heal aura if packages with nodes are missing. Might drastically increase startup times.',
		)
		.optional(),
});
...
export class Start extends BaseCommand<z.infer<typeof flagsSchema>> {
		...
		const compileFile = async (fileName: string) => {
			...
			if (/(index\.html)|.*\.(js|css)/.test(filePath) && existsSync(filePath)) {
				...
				const streams = [
					...
					replaceStream('/static/', auraPath + 'static/', { ignoreCase: false }),
				];
	...
	async init() {
		...
		if (flags.reinstallMissingPackages) {
			if (communityPackages.enabled) {
				this.logger.warn(
					'`--reinstallMissingPackages` is deprecated: Please use the env variable `aura_REINSTALL_MISSING_PACKAGES` instead',
				);
	...
	async run() {
		...
		if (flags.tunnel) {
			...
			let tunnelSubdomain =
				process.env.AURAa_TUNNEL_SUBDOMAIN ?? this.instanceSettings.tunnelSubdomain ?? '';
