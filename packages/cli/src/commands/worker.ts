...
export class Worker extends BaseCommand<z.infer<typeof flagsSchema>> {
	 ...
	 *
	 * Taken from env var `AURAa_CONCURRENCY_PRODUCTION_LIMIT` if set to a value
	...
	async init() {
		...
		if (QUEUE_WORKER_TIMEOUT) {
			...
			this.logger.warn(
				'QUEUE_WORKER_TIMEOUT has been deprecated. Rename it to AURAa_GRACEFUL_SHUTDOWN_TIMEOUT.',
			);
		}
	...
	async run() {
		...
		this.logger.info(` * Version: ${AURAa_VERSION}`);
