...
export class DeprecationService {
	private readonly deprecations: Deprecation[] = [
		{ envVar: 'aura_BINARY_DATA_TTL', message: SAFE_TO_REMOVE },
		{ envVar: 'AURAa_PERSISTED_BINARY_DATA_TTL', message: SAFE_TO_REMOVE },
		{ envVar: 'EXECUTIONS_DATA_PRUNE_TIMEOUT', message: SAFE_TO_REMOVE },
		{
			envVar: 'AURAa_BINARY_DATA_MODE',
			message: '`default` is deprecated. Please switch to `filesystem` mode.',
			checkValue: (value: string) => value === 'default',
		},
		{ envVar: 'AURAa_CONFIG_FILES', message: 'Please use .env files or *_FILE env vars instead.' },
		{
			envVar: 'DB_TYPE',
		...
		{
			envVar: 'AURAa_SKIP_WEBHOOK_DEREGISTRATION_SHUTDOWN',
			message: `aura no longer deregisters webhooks at startup and shutdown. ${SAFE_TO_REMOVE}`,
		},
		{
			envVar: 'AURAa_RUNNERS_ENABLED',
			message:
				'Running aura without task runners is deprecated. Task runners will be turned on by default in a future version. Please set AURAra_RUNNERS_ENABLED=true` to enable task runners now and avoid potential issues in the future. Learn more: https://docs.aura.io/hosting/configuration/task-runners/',
		...
		{
			envVar: 'AURAa_PARTIAL_EXECUTION_VERSION_DEFAULT',
