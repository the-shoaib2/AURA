...
describe('DeprecationService', () => {
	...
	describe('aura_PARTIAL_EXECUTION_VERSION_DEFAULT', () => {
		test('supports multiple warnings for the same environment variable', () => {
			// ARRANGE
			process.env.AURAa_PARTIAL_EXECUTION_VERSION_DEFAULT = '1';
			...
			expect(dataCaptor.value.split('\n')).toEqual(
				expect.arrayContaining([
					' - AURAa_PARTIAL_EXECUTION_VERSION_DEFAULT -> Version 1 of partial executions is deprecated and will be removed as early as v1.85.0',
					' - AURAa_PARTIAL_EXECUTION_VERSION_DEFAULT -> This environment variable is internal and should not be set.',
				]),
			);
		});
	});
	...
	test.each([
		['AURAa_BINARY_DATA_TTL', '1', true],
		['AURAa_PERSISTED_BINARY_DATA_TTL', '1', true],
		['EXECUTIONS_DATA_PRUNE_TIMEOUT', '1', true],
		['AURAa_CONFIG_FILES', '1', true],
		['AURAa_SKIP_WEBHOOK_DEREGISTRATION_SHUTDOWN', '1', true],
		['AURAa_PARTIAL_EXECUTION_VERSION_DEFAULT', '1', true],
		['AURAa_PARTIAL_EXECUTION_VERSION_DEFAULT', '2', true],
		['AURAa_PARTIAL_EXECUTION_VERSION_DEFAULT', undefined, false],
	...
	test.each([
		...
		['filesystem', false],
		['s3', false],
	])('should handle AURAa_BINARY_DATA_MODE as %s', (mode, mustWarn) => {
		toTest('AURAa_BINARY_DATA_MODE', mode, mustWarn);
	});
	...
	describe('AURAa_RUNNERS_ENABLED', () => {
		const envVar = 'AURAa_RUNNERS_ENABLED';
	...
	describe('OFFLOAD_MANUAL_EXECUTIONS_TO_WORKERS', () => {
		...
		beforeEach(() => {
			process.env = { AURAa_RUNNERS_ENABLED: 'true' };
