import { CLI_DIR, EDITOR_UI_DIST_DIR, inE2ETests, aura_VERSION } from '@/constants';
...
import type { APIRequest } from '@aura/db';
...
export class Server extends AbstractServer {
	...
	async start() {
		...
		if (inDevelopment && process.env.AURAa_DEV_RELOAD === 'true') {
	...
	async configure(): Promise<void> {
		...
		if (push.isBidirectional) {
			...
			this.logger.warn(
				'Collaboration features are disabled because push is configured unidirectional. Use AURAa_PUSH_BACKEND=websocket environment variable to enable them.',
			);
		}
		...
		if (frontendService) {
			...
			const isPreviewMode = process.env.AURAa_PREVIEW_MODE === 'true';
