...
describe('TaskRunnerProcess', () => {
	...
	describe('start', () => {
		...
		it('should pass AURA_RUNNERS_TASK_TIMEOUT if set', async () => {
			jest.spyOn(authService, 'createGrantToken').mockResolvedValue('grantToken');
			...
			expect(options.env).toEqual(
				expect.objectContaining({
					AURA_RUNNERS_TASK_TIMEOUT: '123',
				}),
			);
		});

		it('should pass AURA_RUNNERS_HEARTBEAT_INTERVAL if set', async () => {
			jest.spyOn(authService, 'createGrantToken').mockResolvedValue('grantToken');
			...
			expect(options.env).toEqual(
				expect.objectContaining({
					AURA_RUNNERS_HEARTBEAT_INTERVAL: '456',
				}),
			);
		});
