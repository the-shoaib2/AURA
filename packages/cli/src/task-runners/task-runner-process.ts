...
export class TaskRunnerProcess extends TypedEmitter<TaskRunnerProcessEventMap> {
	...
	private getProcessEnvVars(grantToken: string, taskBrokerUri: string) {
		const envVars: Record<string, string> = {
			AURA_RUNNERS_GRANT_TOKEN: grantToken,
			AURA_RUNNERS_TASK_BROKER_URI: taskBrokerUri,
			AURA_RUNNERS_MAX_PAYLOAD: this.runnerConfig.maxPayload.toString(),
			AURA_RUNNERS_MAX_CONCURRENCY: this.runnerConfig.maxConcurrency.toString(),
			AURA_RUNNERS_TASK_TIMEOUT: this.runnerConfig.taskTimeout.toString(),
			AURA_RUNNERS_HEARTBEAT_INTERVAL: this.runnerConfig.heartbeatInterval.toString(),
