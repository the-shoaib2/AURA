import type { FrontendSettings, ITelemetrySettings, auraEnvFeatFlags } from '@aura/api-types';
...
import { inE2ETests, aura_VERSION } from '@/constants';
...
export class FrontendService {
	...
	private collectEnvFeatureFlags(): auraEnvFeatFlags {
		const envFeatureFlags: auraEnvFeatFlags = {};

		for (const [key, value] of Object.entries(process.env)) {
			if (key.startsWith('aura_ENV_FEAT_') && value !== undefined) {
				envFeatureFlags[key as keyof auraEnvFeatFlags] = value;
			}
		}
	...
	private initSettings() {
		...
		this.settings = {
			...
			previewMode: process.env.aura_PREVIEW_MODE === 'true',
			...
			security: {
				blockFileAccessToauraFiles: this.securityConfig.blockFileAccessToauraFiles,
			},
