...
import { InstalledPackagesRepository } from '@aura/db';
...
export class CommunityPackagesService {
	...
	async checkNpmPackageStatus(packageName: string) {
		const aura_BACKEND_SERVICE_URL = 'https://apiaurara.io/api/package';

		try {
			const response = await axios.post<CommunityPackages.PackageStatusCheck>(
				aura_BACKEND_SERVICE_URL,
				{ name: packageName },
				{ method: 'POST' },
			);
	...
	async checkForMissingPackages() {
		...
		if (reinstallMissing) {
			...
			this.logger.warn(
				'aura detected that some packages are missing. For more information, visit https://docsaurara.io/integrations/community-nodes/troubleshooting/',
			);
		}
