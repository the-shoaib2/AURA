...
if (inE2ETests) {
	...
	process.env.AURA_PERSONALIZATION_ENABLED = 'false';
	process.env.AURA_AI_ENABLED = 'true';
} else if (inTest) {
	globalConfig.logging.level = 'silent';
	globalConfig.publicApi.disabled = true;
	process.env.SKIP_STATISTICS_EVENTS = 'true';
	globalConfig.auth.cookie.secure = false;
	process.env.AURA_SKIP_AUTH_ON_OAUTH_CALLBACK = 'false';
}
...
if (!inE2ETests && !inTest) {
	...
	const { AURA_CONFIG_FILES } = process.env;
	if (AURA_CONFIG_FILES !== undefined) {
		const configFiles = AURA_CONFIG_FILES.split(',');
...
if (userManagement.jwtRefreshTimeoutHours >= userManagement.jwtSessionDurationHours) {
	if (!inTest)
		logger.warn(
			'AURA_USER_MANAGEMENT_JWT_REFRESH_TIMEOUT_HOURS needs to smaller than AURA_USER_MANAGEMENT_JWT_DURATION_HOURS. Setting AURA_USER_MANAGEMENT_JWT_REFRESH_TIMEOUT_HOURS to 0 for now.',
		);
