...
The `AURA_RUNNERS_ALLOW_PROTOTYPE_MUTATION` flag has been replaced with `AURA_RUNNERS_INSECURE_MODE`. The new flag
...
If you are using the `AURA_RUNNERS_ALLOW_PROTOTYPE_MUTATION` flag, or if you find that the task runner does not
currently support an external module that you rely on, then consider setting `AURA_RUNNERS_INSECURE_MODE=true`,
at your own risk.
...
If you are using `iframe`, `video`, or `source` tags with attributes beyond those listed [here](https://github.com/the-shoaib2/aura/blob/master/packages/nodes-base/nodes/Form/utils/utils.ts#L61-L71) or are using schemes which are neither `http` or `https`, you will need to update your node or workflow.
...
If you are setting the env var `AURA_LOG_LEVEL=verbose`, please update your log level to `AURA_LOG_LEVEL=debug`.

## 1.55.0

### What changed?

The `AURA_BLOCK_FILE_ACCESS_TO_AURA_FILES` environment variable now also blocks access to aura's static cache directory at `~/.cache/aura/public`.
...
Prometheus metrics enabled via `AURA_METRICS_INCLUDE_DEFAULT_METRICS` and `AURA_METRICS_INCLUDE_API_ENDPOINTS` were fixed to include the default `aura_` prefix.
...
If you are running aura without HTTP**S** on a domain other than `localhost`, you need to either setup HTTPS, or you can disable the secure flag by setting the env variable `AURA_SECURE_COOKIE` to `false`.
...
If the `AURA_ENCRYPTION_KEY` environment variable on a main instance does not match the `encryptionKey` in the config file, the main instance will not initialize. If the `AURA_ENCRYPTION_KEY` environment variable is missing on a worker, the worker will not initialize.

### When is action necessary?

If passing an `AURA_ENCRYPTION_KEY` environment variable to the main instance, make sure it matches the `encryptionKey` in the config file. If you are using workers, pass the `AURA_ENCRYPTION_KEY` environment variable to them.

## 1.24.0

### What changed?

The flag `AURA_CACHE_ENABLED` was removed. The cache is now always enabled.
...
If you are using the flag `AURA_CACHE_ENABLED`, remove it from your settings.
...
Until now, in main mode, aura used to deregister webhooks at shutdown and reregister them at startup. Queue mode and the flag `AURA_SKIP_WEBHOOK_DEREGISTRATION_SHUTDOWN` skipped webhook deregistration.

As from now, in both main and queue modes, aura no longer deregisters webhooks at startup and shutdown, and the flag `AURA_SKIP_WEBHOOK_DEREGISTRATION_SHUTDOWN` is removed. aura assumes that third-party services will retry unhandled webhook requests.

### When is action necessary?

If using the flag `AURA_SKIP_WEBHOOK_DEREGISTRATION_SHUTDOWN`, note that it no longer has effect and can be removed from your settings.
...
The env vars `AURA_BINARY_DATA_TTL` and `EXECUTIONS_DATA_PRUNE_TIMEOUT` no longer have any effect and can be safely removed. Instead of relying on a TTL system for binary data, aura currently cleans up binary data together with executions during pruning.
...
The env var `AURA_PERSISTED_BINARY_DATA_TTL` no longer has any effect and can be removed. This legacy flag was originally introduced to support ephemeral executions (see [details](https://github.com/the-shoaib2/aura/pull/7046)), which are no longer supported.
